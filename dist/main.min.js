...
b=>{this.saveSettings()});document.addEventListener("_auth_id",async({detail:{authId:b,authTypeId:c,isAuthorized:d}})=>{function e(l){this.lm=this.lm.filter(m=>1<<m.id&l?(m.disabled||(window[m.name].INIT.apply(this.rootThis,[this.root]),window[m.name].toggleSettingsVisibility=this.toggleSettingsVisibility(m),m.initialized=!0),m.locked=!1):m.locked=!0);this.renderModulesList()}async function g(l,m){m=(new TextEncoder).encode(m);var p=await crypto.subtle.digest("SHA-256",m);m=window.atob(l).slice(0,
12);m={name:"AES-GCM",iv:new Uint8Array(Array.from(m).map(n=>n.charCodeAt(0)))};p=await crypto.subtle.importKey("raw",p,m,!1,["decrypt"]);l=window.atob(l).slice(12);l=new Uint8Array(Array.from(l).map(n=>n.charCodeAt(0)));try{const n=await crypto.subtle.decrypt(m,p,l);return(new TextDecoder).decode(n)}catch(n){throw Error("Decrypt failed");}}function f(l){document.dispatchEvent(new CustomEvent("_auth_perms",{detail:{ps:l}}))}if(this.lm.length){var h=GPModulesManager_.DEFAULT_PERMISSIONS,k=null;try{if(!c)throw new GPAuthError_;
...
this.edition.classList.remove("hidden")}setEditionState(a,b){this.edition.dataset.state=a;this.edition.title="";a===GPModulesManager_.EDITION_STATE.TIME_LEFT?this.edition.dataset.timeLeft=this.formatTimeLeft(b):a===GPModulesManager_.EDITION_STATE.EXPIRED&&(this.edition.title="\u041e\u0444\u043e\u0440\u043c\u0438\u0442\u044c \u043f\u043e\u0434\u043f\u0438\u0441\u043a\u0443 \u043d\u0430 \u043c\u043e\u0434 \u043d\u0430 Boosty")}formatTimeLeft(a){a=Math.trunc(a/864E5);const b=a%100;var c=b%10;let d;if(10<=
b&&20>b)d=2;else if(1===c)d=0;else if(1<c&&5>c)d=1;else if(!c||4<c)d=2;c=["\u041e\u0441\u0442\u0430\u043b\u0441\u044f","\u041e\u0441\u0442\u0430\u043b\u043e\u0441\u044c"];const e=["\u0434\u0435\u043d\u044c","\u0434\u043d\u044f","\u0434\u043d\u0435\u0439"];return`${d?c[1]:c[0]} ${1>b?"\u043c\u0435\u043d\u044c\u0448\u0435 \u0434\u043d\u044f":`${a} ${e[d]}`}`}onMListClick(a){2===a.button?a.preventDefault():a.target.matches(".module-title_, .module-settings_")&&!a.target.classList.contains("disabled_")&&
