...
e=e.slice(m)}var u=k.length;k.forEach(function(r,n){g(a(r,f,!0),n,u)})}var c=Object.keys({open:0,close:1,ping:2,pong:3,message:4,upgrade:5,noop:6}),d={type:"error",data:"parser error"};return{decodePayload:function(e,f,g){if("string"!==typeof e)return b(e,f,g);"function"===typeof f&&(f=null);if(""===e)return d;g="";for(var k,h,l=0,m=e.length;l<m;l++)if(h=e.charAt(l),":"!==h)g+=h;else{if(""===g||g!=(k=Number(g)))return d;h=e.substr(l+1,k);if(g!=h.length)return d;if(h.length)return e=a(h,f,!1),d.type===
...
w(197)+w(227),"",w(225)+"U=",w(217)+"NlTWVtb3J5",w(200)+(w(226)+w(213)),w(211)+(w(199)+"b25z"),w(215)+"pvbmU=",w(204)+"=="][w(201)](window[w(222)]),B=JSON.stringify([window[q[0]][q[1]]?.[q[4]],window[q[0]][q[5]],window[q[6]][q[7]],window[q[6]][q[8]],window[q[0]][q[9]],window[q[0]][q[4]],window[q[0]][q[11]],window[q[0]][q[12]],(new (window[w(208)]?.[q[13]]))[q[14]]()[q[15]]]);let v=-559038803,t=1103547925;for(let y=0,z;y<B.length;y++)z=B[w(218)](y),v=Math[q[16]](v^z,2654435761),t=Math[q[16]](t^z,1597334677);
return v=Math[q[16]](v^v>>>16,2246822507)^Math[q[16]](t^t>>>13,3266489909),t=Math[q[16]](t^t>>>16,2246822507)^Math[q[16]](v^v>>>13,3266489909),String(4294967296*(2097151&t)+(v>>>0))}this.s.allowAnalytics||(this.setEditionState(GPModulesManager_.STATE.ANALYTICS),await this.waitForAnalyticsConsent());let m=!1,p=!1,u=!1,r={};if(this.lm.length){var n=GPModulesManager_.DEFAULT_PERMISSIONS,A=null;try{if(!a||!this.publicKey)throw A=[GPModulesManager_.STATE.KEY],new GPAuthError_;if(!d)if(this.s.twitchAuth){d=
...
!0;if(x.ed<Date.now())throw u=!0,A=[GPModulesManager_.STATE.EXPIRED],2&x.p&&(n|=16),new GPAuthError_;A=[GPModulesManager_.STATE.TIME_LEFT,x.ed-Date.now()]}else r.g||(A=[GPModulesManager_.STATE.SUBSCRIPTION]);A&&this.setEditionState(...A);f.call(this,n|x.p);k.call(this,n|x.p);this.renderModulesList();m=!0}catch(C){C instanceof GPAuthError_||C instanceof GPDecryptError_||console.error(C),A||(e?A??=[GPModulesManager_.STATE.SUBSCRIBE]:this.publicKey&&(A??=[GPModulesManager_.STATE.AUTH])),A&&this.setEditionState(...A),
f.call(this,n),k.call(this,n)}this.dispatchEvent(new CustomEvent("modules_loaded",{detail:{decryptedData:r,authTypeId:d,isAuthorized:e,isSubscribed:m,kd:p,kt:u}}));this.handleSubExpiration(A)}else this.dispatchEvent(new CustomEvent("modules_loaded",{detail:{decryptedData:r,authTypeId:d,isAuthorized:e,isSubscribed:m,kd:p,kt:u}}))});document.addEventListener("gp:act.auth_key_changed",c=>{this.container.classList.add("reload-required_")});document.addEventListener("gp:act.auth_key",({detail:{publicKey:c}})=>
...
{d.onsuccess=f=>{const g=f.target.result;f=this.filterDBStores(g).map(k=>new Promise(h=>{const l=g.transaction(k,"readonly").objectStore(k);l.getAll().onsuccess=m=>{h({name:l.name,keyPath:l.keyPath,autoIncrement:l.autoIncrement,indexes:[...l.indexNames].map(p=>{const {name:u,keyPath:r,unique:n,multiEntry:A}=l.index(p);return{name:u,keyPath:r,unique:n,multiEntry:A}}),entries:m.target.result})}}));Promise.all(f).then(k=>e({name:g.name,version:g.version,stores:k}))}})});Promise.all(b).then(async c=>
...
k=>{const h=k.target.result;g.map(l=>{const m=h.createObjectStore(l.name,{keyPath:l.keyPath,autoIncrement:l.autoIncrement});l.indexes.forEach(p=>{const {name:u,keyPath:r,...n}=p;m.createIndex(u,r,n)})})};f.onsuccess=k=>{const h=k.target.result;k=g.map(l=>new Promise(m=>{const p=h.transaction(l.name,"readwrite");p.oncomplete=m;const u=p.objectStore(l.name);u.clear().onsuccess=()=>{l.entries.forEach((r,n)=>{u.add(r,l.keyPath?void 0:n)})}}));Promise.all(k).then(d)};f.onerror=e})),await Promise.allSettled(a));
...
b.appendChild(c);this.handleControlsScrollbar(d);this.handleControlsScrollHints(d);return c}buildView(a,b=null){const {module:c,storage:d,settings:e,ui:f}=this.viewsData.get(a);b?b.innerHTML="":(b=document.createElement("div"),b.className="gp-settings_",b.dataset.viewIndex=this.views.size,b.scrollPos=0,b.addEventListener("animationend",u=>{"gp-set-highlight_"===u.animationName&&u.target.classList.remove("highlight_")}),(new ResizeObserver(u=>{u[0].contentRect.height&&this.updateScrollPos(b)})).observe(b),
this.views.add(b));b.moduleName=a;b.labels=new Map;const g=document.createElement("div");g.className="controls_ gp-ui-scrollbar_";g.view=b;g.addEventListener("scroll",u=>{b.scrollPos=u.target.scrollTop;this.handleControlsScrollHints(u.target)});b.appendChild(g);b.controls=g;var k=document.createElement("div");k.className="buttons_";b.appendChild(k);const h=document.createElement("div");h.className="load-btn_ btn_ gp-ui-btn_";h.textContent="\ud83e\udc45";h.title=this.l.LOAD_JSON_TTL;h.addEventListener("click",
()=>{b.classList.toggle("json-data_");l.focus()});h.addEventListener("animationend",u=>{u.target.classList.remove("failure_");u.target.classList.remove("success_")});k.appendChild(h);const l=document.createElement("input");l.className="json-data-input_ gp-ui-input_";l.placeholder=this.l.JSON_INPUT_PH;l.spellcheck=!1;l.autocomplete="off";l.addEventListener("keydown",u=>{"Escape"===u.key&&(b.classList.remove("json-data_"),u.target.blur())});l.addEventListener("input",u=>{this.jsonDataInputHandler(a,
u.target,h)});GPUtils_.setInputFocusBlurHandler(l);k.appendChild(l);var m=document.createElement("div");m.className="restore-btn_ btn_ gp-ui-btn_";m.textContent=this.l.RESTORE_BTN;m.title=this.l.RESTORE_BTN_TTL;m.addEventListener("click",()=>{this.restore(a)});k.appendChild(m);m=document.createElement("div");m.className="default-btn_ btn_ gp-ui-btn_";m.textContent=this.l.RESET_BTN;m.title=this.l.RESET_BTN_TTL;m.addEventListener("click",()=>{this.reset(a)});k.appendChild(m);k=document.createDocumentFragment();
for(let u in f)if(m=f[u]??{},this.isLabel(u)){var p=document.createElement("div");p.className="label_";p.textContent=m.text;k.appendChild(p);b.labels.set(m.text,p)}else if(this.isSeparator(u))p=document.createElement("div"),p.className="separator_",p.classList.toggle(`${m.size}_`,!!m.size),k.appendChild(p);else if(m.type&&!m.hidden)switch(m.type){case "component":p=document.createElement("div");p.className="component_";p.dataset.name=u;p.appendChild(m.getElement());k.appendChild(p);this.setControlStateAndDependents(f,
u,e,p);break;case "buttons":const r=document.createElement("div");r.className="buttons_";r.dataset.name=u;m.getButtons().forEach(A=>{A.classList.add("btn_");r.appendChild(A)});k.appendChild(r);this.setControlStateAndDependents(f,u,e,r);break;default:p=document.createElement("div");p.className="setting_";p.dataset.name=u;const n=GPUtils_.buildTooltip(m.description);p.appendChild(n);m=this.buildName(u,m.description);p.appendChild(m);m=this.buildControl(f,u,e[u],c,d,e,g);p.appendChild(m);k.appendChild(p);
this.setControlStateAndDependents(f,u,e,p)}g.appendChild(k);return{view:b,controls:g}}updateView(a,b=null){const {controls:c}=this.buildView(b??a.moduleName,a);this.handleControlsScrollbar(c);this.handleControlsScrollHints(c);this.updateScrollPos(a,b?0:a.scrollPos)}updateScrollPos(a,b){a.controls.scrollTop=b??a.scrollPos}handleControlsScrollbar(a){a.scrollHeight>a.clientHeight&&(a.parentElement.clientHeight-41>a.scrollHeight?a.style.paddingBottom=`${a.scrollHeight-a.clientHeight}px`:a.classList.add("scrollbar_"))}handleControlsScrollHints(a){a.parentElement.classList.toggle("scroll-hint-top_",
...
args:h,formatter:l,l10n:m,inputTrigger:p,beforeActivation:u}=a[b],r=document.createElement("div");r.className="control_";let n;switch(k){case "switch":n=document.createElement("div");n.className="gp-ui-switch_";var A=`ctrl-${g.view.dataset.viewIndex}-${b}_`;const C=document.createElement("input");C.id=A;C.type="checkbox";C.name=b;C.checked=c;if(u){let D=!1;C.addEventListener("click",async G=>{G.preventDefault();if(!D){if(C.checked&&(D=!0,G=await u(),D=!1,!G))return;requestAnimationFrame(()=>{C.checked=
!C.checked;C.dispatchEvent(new Event("change",{bubbles:!0}))})}})}n.appendChild(C);const x=document.createElement("label");x.setAttribute("for",A);n.appendChild(x);r.appendChild(n);break;case "slider":const {min:w,max:q,step:B,reverse:v}=h;n=document.createElement("input");n.className="gp-ui-slider_";n.type="range";n.min=w;n.max=q;n.step=B;n.value=c;n.name=b;v&&(n.style.direction="rtl");n.addEventListener("input",D=>{t.textContent=l?l(Number.parseFloat(D.target.value)):D.target.value});const t=document.createElement("div");
t.className="gp-ui-slider-preview_";t.textContent=l?l(c):c;t.addEventListener("pointerdown",D=>{let G=D.clientY;const H=K=>{var J=K.clientY-G;const F=0>J;Math.abs(J)>=GPSettingsManager_.POINTER_SLIDER_PREVIEW_CHANGE_SENSITIVITY&&(J=n.valueAsNumber,J+=(v?!F:F)?B:-B,J=Math.max(Math.min(J,q),w),J=Math.round(1E3*J)/1E3,n.value=J,t.textContent=l?l(J):J,n.dispatchEvent(new Event("change",{bubbles:!0})),G=K.clientY)},L=K=>{const J=0>K.deltaY;let F=n.valueAsNumber;F+=(v?!J:J)?B:-B;F=Math.max(Math.min(F,q),
w);F=Math.round(1E3*F)/1E3;n.value=F;t.textContent=l?l(F):F;n.dispatchEvent(new Event("change",{bubbles:!0}));K.preventDefault();K.stopPropagation();K.stopImmediatePropagation()};D.currentTarget.setPointerCapture(D.pointerId);D.currentTarget.addEventListener("pointermove",H);D.currentTarget.addEventListener("pointerup",K=>{K.currentTarget.removeEventListener("pointermove",H);document.removeEventListener("wheel",L,{passive:!1});n.focus()},{once:!0});document.addEventListener("wheel",L,{passive:!1})});
r.appendChild(n);r.appendChild(t);break;case "list-numbers":({items:A}=h);n=document.createElement("form");n.className="gp-ui-list-n_";n.value=c;n.name=b;A.forEach(({min:D,max:G,step:H,reverse:L},K)=>{const J=document.createElement("input");J.type="range";J.value=c[K];J.name=b;n.appendChild(J);const F=document.createElement("div");F.className="gp-ui-slider-preview_";F.textContent=l?l(c[K]):c[K];F.addEventListener("animationend",O=>{"list-n-item-highlight_"!==O.animationName&&"list-n-item-fadeout_"!==
...
[h,l,m]=GPColorUtils_.hsvToRgb(Math.max(f-.04246122552758713,0),Math.max(g-.012523565849717255,0),Math.max(k-.05882352941176472,0));this.style.setProperty("--gp-panels-elems2-color",`rgb(${h}, ${l}, ${m})`);this.colorPanel?.updatePanelsBackground(a)}updatePanelsElements(a,b){const c=parseInt(a.substr(1,2),16),d=parseInt(a.substr(3,2),16);a=parseInt(a.substr(5,2),16);this.style.setProperty("--gp-panels-elems-rgb",`${c} ${d} ${a}`);this.style.setProperty("--gp-panels-elems-rgb-opacity",`${b}%`)}updateControlsBackgroundBlurLevel(a){this.style.setProperty("--gp-panels-controls-backdrop-filter",
...
document.removeEventListener("keyup",g)),b.keyUp?.(),this.container.removeEventListener("pointerdown",h),c=!1)};let k=m=>{};const h=(m,p=!1)=>{if(!this.isDrawing&&this.activator===a&&m.buttons&7){if(!p){if(1===m.button&&b.middleButton){b.middleButton(m);return}if(2===m.button&&b.rightButton){b.rightButton(m);return}}var u=p?0:m.button;b.pointer?.[u]?.hide&&this.container.classList.add("no-pointer_");b.pointer?.[u]?.pointerDown?.(m);b.pointer?.[u]?.pointerMove&&(k=b.pointer[u].pointerMove(m));this.container.setPointerCapture(m.pointerId);
...
e.height);d.drawImage(this.lastDraw,0,0,e.width,e.height);d.drawImage(this.canvas,0,0,e.width,e.height)}else d=this.canvasCtx;return this.fillAlgorithm(d,a,b,c,this.canvas.width,this.canvas.height).map((e,f)=>4>f%5?Math.round(e/this.density):e)}setFillAlgorithm(a){switch(a){case GPPainter_.FILL_ALGORITHM.OPTIMIZED:this.fillAlgorithm=this.getFillRectsOptimized;break;default:this.fillAlgorithm=this.getFillRects}}getFillRects(a,b,c,d,e,f){let g;var k=(q,B,v)=>{let t=B,y=v;if(!p(B,v,q))return{w:-1,h:-1};
for(;p(t+1,y,q);)t++;let z=t;do for(t=B-1,y++;p(t+1,y,q)&&t+1<=z;)t++;while(t==z);q=--y-v;return{x:B,y:v,w:z-B,h:q}},h=(q,B,v)=>{let t=B,y=v;if(!p(B,v,q))return{w:-1,h:-1};for(;p(t-1,y,q);)t--;let z=t;do for(t=B+1,y--;p(t-1,y,q)&&t-1>=z;)t--;while(t==z);q=v-++y;return{x:z,y,w:B-z,h:q}},l=(q,B,v)=>{let t=B,y=v;if(!p(B,v,q))return{w:-1,h:-1};for(;p(t,y+1,q);)y++;let z=y;do for(y=v-1,t--;p(t,y+1,q)&&y+1<=z;)y++;while(y==z);q=B-++t;return{x:t,y:v,w:q,h:z-v}},m=(q,B,v)=>{let t=B,y=v;if(!p(B,v,q))return{w:-1,
h:-1};for(;p(t,y-1,q);)y--;let z=y;do for(y=v+1,t++;p(t,y-1,q)&&y-1>=z;)y--;while(y==z);q=--t-B;return{x:B,y:z,w:q,h:v-z}};let p=(q,B,v)=>{if(g[q][B])return!1;q=4*(q+B*e);q=C.slice(q,q+4);B=Math.abs(v[1]-q[1]);let t=Math.abs(v[2]-q[2]),y=Math.abs(v[3]-q[3]);return 20>Math.abs(v[0]-q[0])&&20>B&&20>t&&20>y||0==q[3]&&255==v[0]&&255==v[1]&&255==v[2]||0==v[3]&&255==q[0]&&255==q[1]&&255==q[2]};var u=q=>{var B=q[0];let v=q[1],t=B+q[2],y=v+q[3];for(;B<t;B++)for(let z=v;z<y;z++)g[B][z]=!0;return q};b=Math.round(b);
...
b=[...u([r.x,r.y,r.w+1,r.h+1]),0];do{c=0;for(2==n.ref&&(c+=n.andada);c<=n.h;)-1!=(x=(r=m(a,n.x+n.w+1,n.y+n.h-c)).h)?(d[x].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:1,andada:n.h+1-c,stack:b.length}),b.push(...u([r.x,r.y,r.w+1,r.h+1]),n.stack),x>A&&(A=x),c+=x):c++;c=0;for(1==n.ref&&(c+=n.andada);c<=n.h;)-1!=(x=(r=l(a,n.x-1,n.y+c)).h)?(d[x].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:2,andada:n.h+1-c,stack:b.length}),b.push(...u([r.x,r.y,r.w+1,r.h+1]),n.stack),x>A&&(A=x),c+=x):c++;c=0;for(4==n.ref&&(c+=n.andada);c<=n.w;)-1!=
(x=(r=k(a,n.x+c,n.y+n.h+1)).w)?(d[x].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:3,andada:n.w+1-c,stack:b.length}),b.push(...u([r.x,r.y,r.w+1,r.h+1]),n.stack),x>A&&(A=x),c+=x):c++;c=0;for(3==n.ref&&(c+=n.andada);c<=n.w;)-1!=(x=(r=h(a,n.x+n.w-c,n.y-1)).w)?(d[x].push({x:r.x,y:r.y,w:r.w,h:r.h,ref:4,andada:n.w+1-c,stack:b.length}),b.push(...u([r.x,r.y,r.w+1,r.h+1]),n.stack),x>A&&(A=x),c+=x):c++;for(n=d[A].pop();null==n&&0<A;)n=d[--A].pop()}while(null!=n);let w=new Map;k=(q,B,v,t,y)=>{if(!g[q][B]){v=q+v;var z=B+
t;if(!(0>v||v>=e||0>z||z>=f))for(t=4*(q+B*e),v=4*(v+z*e),z=0;4>z;z++)if(20<=Math.abs(C[t+z]-C[v+z])){t=q*f+B;w.has(t)||w.set(t,[q,B,y]);break}}};for(h=0;h<b.length;h+=5){l=b[h];m=b[h+1];u=b[h+2];x=b[h+3];if(l+u<e)for(c=m;c<m+x;c++)k(l+u,c,1,0,h);if(0<l)for(c=m;c<m+x;c++)k(l-1,c,-1,0,h);if(m+x<f)for(c=l;c<l+u;c++)k(c,m+x,0,1,h);if(0<m)for(x=l;x<l+u;x++)k(x,m-1,0,-1,h)}for(let [,[q,B,v]]of w)b.push(q,B,1,1,v);return b}getFillRectsOptimized(a,b,c,d,e,f){b=Math.round(b);c=Math.round(c);const g=new Uint8Array(e*
f),k=new Uint8Array(e*f);var h=parseInt(d.replace("#","0x"));d=h%256;var l=(h=Math.floor(h/256))%256;h=Math.floor(h/256);const m=a.getImageData(0,0,e,f).data;a=4*(b+c*e);const p=m[a],u=m[a+1],r=m[a+2],n=m[a+3],A=(t,y,z,E,I,Q)=>{if(0>t||t>=e||0>y||y>=f)return!1;t=y*e+t;if(g[t])return!1;t*=4;y=z-m[t];const D=E-m[t+1],G=I-m[t+2],H=Q-m[t+3];return-20<y&&20>y&&-20<D&&20>D&&-20<G&&20>G&&-20<H&&20>H||0===m[t+3]&&255===z&&255===E&&255===I||0===Q&&255===m[t]&&255===m[t+1]&&255===m[t+2]};a=t=>{const y=t[0];
var z=t[1];const E=y+t[2],I=z+t[3];for(;z<I;z++){const Q=z*e;for(let D=y;D<E;D++)g[Q+D]=1}return t};if(A(b,c,h,l,d,255))return[];for(;A(b-1,c,p,u,r,n);)b--;for(;A(b,c-1,p,u,r,n);)c--;d=(t,y)=>{let z=t,E=y;if(!A(t,y,p,u,r,n))return{w:-1,h:-1};for(;A(z+1,E,p,u,r,n);)z++;let I=z;do for(z=t-1,E++;A(z+1,E,p,u,r,n)&&z+1<=I;)z++;while(z===I);return{x:t,y,w:I-t,h:--E-y}};l=(t,y)=>{let z=t,E=y;if(!A(t,y,p,u,r,n))return{w:-1,h:-1};for(;A(z-1,E,p,u,r,n);)z--;let I=z;do for(z=t+1,E--;A(z-1,E,p,u,r,n)&&z-1>=I;)z--;
while(z===I);return{x:I,y:++E,w:t-I,h:y-E}};h=(t,y)=>{let z=t,E=y;if(!A(t,y,p,u,r,n))return{w:-1,h:-1};for(;A(z,E+1,p,u,r,n);)E++;let I=E;do for(E=y-1,z--;A(z,E+1,p,u,r,n)&&E+1<=I;)E++;while(E===I);return{x:++z,y,w:t-z,h:I-y}};var C=(t,y)=>{let z=t,E=y;if(!A(t,y,p,u,r,n))return{w:-1,h:-1};for(;A(z,E-1,p,u,r,n);)E--;let I=E;do for(E=y+1,z++;A(z,E-1,p,u,r,n)&&E-1>=I;)E--;while(E===I);return{x:t,y:I,w:--z-t,h:y-I}},x=[];for(var w=0;w<=e;w++)x[w]=[];w=d(b,c);b={x:b,y:c,w:w.w,h:w.h,ref:0,andada:0,stack:0};
...
(w+=b.andada);w<=b.w;)v=d(b.x+w,b.y+b.h+1),B=v.w,-1!==B?(x[B].push({x:v.x,y:v.y,w:v.w,h:v.h,ref:3,andada:b.w+1-w,stack:q.length}),q.push(...a([v.x,v.y,v.w+1,v.h+1]),b.stack),B>c&&(c=B),w+=B):w++;w=0;for(3===b.ref&&(w+=b.andada);w<=b.w;)v=l(b.x+b.w-w,b.y-1),B=v.w,-1!==B?(x[B].push({x:v.x,y:v.y,w:v.w,h:v.h,ref:4,andada:b.w+1-w,stack:q.length}),q.push(...a([v.x,v.y,v.w+1,v.h+1]),b.stack),B>c&&(c=B),w+=B):w++;for(b=x[c].pop();null==b&&0<c;)b=x[--c].pop()}while(null!=b);a=(t,y,z,E,I)=>{const Q=y*e+t;if(!(g[Q]||
0>z||z>=e||0>E||E>=f)){var D=4*(t+y*e),G=4*(z+E*e);z=m[D]-m[G];E=m[D+1]-m[G+1];var H=m[D+2]-m[G+2];D=m[D+3]-m[G+3];(-20>=z||20<=z||-20>=E||20<=E||-20>=H||20<=H||-20>=D||20<=D)&&0===k[Q]&&(k[Q]=1,q.push(t,y,1,1,I))}};d=q.length;for(l=0;l<d;l+=5){h=q[l];C=q[l+1];x=q[l+2];c=q[l+3];b=l;if(h+x<e)for(w=C;w<C+c;w++)a(h+x,w,h+x+1,w,b);if(0<h)for(w=C;w<C+c;w++)a(h-1,w,h-2,w,b);if(C+c<f)for(w=h;w<h+x;w++)a(w,C+c,w,C+c+1,b);if(0<C)for(c=h;c<h+x;c++)a(c,C-1,c,C-2,b)}return q}setTimer(){this.timer&&(this.timer.reset(),
...
a.button){var b=this.frame.dataset.anchor,c=this.offsetX,d=this.offsetY,[e,f,g,k]=this.getCoords(a),h=(l=>{const [m,p,u]=this.getStickyCoords(l,l.pageX-e,l.pageY-f,g,k);this.setPosition(m,p,g,k,u)}).bind(this);this.frame.setPointerCapture(a.pointerId);this.frame.addEventListener("pointermove",h);this.frame.addEventListener("pointerup",()=>{this.frame.removeEventListener("pointermove",h);this.isChanged||this.offsetX===c&&this.offsetY===d||this.handleChange();this.fm.isSingleFrame()&&(this.fm.x=this.offsetX,
this.fm.y=this.offsetY,this.dispatchPos(this.offsetX,this.offsetY),this.staticAnchor||this.frame.dataset.anchor===b||(this.fm.anchor=this.frame.dataset.anchor,this.dispatchAnchor(this.frame.dataset.anchor)))},{once:!0});(this.fm.stickyEdges&&a.ctrlKey||!this.fm.stickyEdges&&!a.ctrlKey)&&h(a)}}getStickyCoords(a,b,c,d,e){if(this.fm.stickyEdges&&a.ctrlKey||!this.fm.stickyEdges&&!a.ctrlKey||!this.fm.hasFrames())return[b,c,null];const f=this.fm.stickyEdgesDistance;let g,k,h,l,m=null,p,u;this.fm.getFrames().forEach(r=>
{if(r!==this){var n=r.frame.getBoundingClientRect();var A=Math.abs(n.y-(c+e));A<f&&(void 0===h||A<h)&&n.x+n.width>=b&&n.x<=b+d&&(!u||u[0]===r.frame.dataset.anchor[0])&&(h=A,c=n.y-e,m=u=r.frame.dataset.anchor);A=Math.abs(n.y+n.height-c);A<f&&(void 0===l||A<l)&&n.x+n.width>=b&&n.x<=b+d&&(!u||u[0]===r.frame.dataset.anchor[0])&&(l=A,c=n.y+n.height,m=u=r.frame.dataset.anchor);A=Math.abs(n.x-(b+d));A<f&&(void 0===g||A<g)&&n.y+n.height>=c&&n.y<=c+e&&(!p||p[1]===r.frame.dataset.anchor[1])&&(g=A,b=n.x-d,m=
...
[];const l=e,m=Math.floor(self.data.length/l);g=new OffscreenCanvas(g,k);k=g.getContext("2d",{willReadFrequently:!0});let p;self.background&&(k.fillStyle=d,k.fillRect(0,0,g.width,g.height),self.draw(k,this.background,f,!1),p=k.getImageData(0,0,g.width,g.height),k.clearRect(0,0,g.width,g.height));for(let u=0;u<m;u++){var h=u*l;h=self.data.slice(h,h+l);self.draw(k,h,f,self.eraserAlpha);h=k.getImageData(0,0,g.width,g.height);this.cache[f].push(h)}self.background&&this.cache[f].push(p)}}let b,c,d,e;self.data;
