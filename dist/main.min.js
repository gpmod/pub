...
1),this.sendQueueTimeLeft-=GPProxy_.SEND_QUEUE_DELAY):this.sendQueue.push([c,a,f])}else this.sendQueue.push([c,a,b.v[1]]),this.sendQueueTimeLeft+=GPProxy_.SEND_QUEUE_DELAY;this.sendInProcess||this.delayedSend()}else this.ws.send(c),this.lastSendTime=Date.now()}delayedSend(a=GPProxy_.SEND_QUEUE_DELAY){if(this.sendQueue.length){this.sendInProcess=!0;var b=Date.now()-this.lastSendTime;if(b>a||this.sendQueue[0][1]){const [c,d]=this.sendQueue.shift();this.sendQueueTimeLeft-=d?0:GPProxy_.SEND_QUEUE_DELAY;
this.ws.send(c);this.lastSendTime=Date.now();this.dispatchEvent(new CustomEvent("send_queue_updated",{detail:{size:this.sendQueue.length,timeLeft:this.sendQueueTimeLeft}}));0<this.sendQueue.length&&(this.sendQueueTimer=setTimeout(()=>{this.delayedSend()},d?0:a));this.sendInProcess=!1}else this.sendQueueTimer=setTimeout(()=>{this.delayedSend()},a-b)}else this.sendInProcess=!1,this.sendQueueTimeLeft=0,this.dispatchEvent(new CustomEvent("send_queue_updated",{detail:{size:this.sendQueue.length,timeLeft:this.sendQueueTimeLeft}}))}resetSendQueue(){clearTimeout(this.sendQueueTimer);
this.sendQueue=[];this.sendInProcess=!1;this.sendQueueTimeLeft=0}getSendQueueSize(){return this.sendQueue.length}kickPlayer(a){this.send(GPProxy_.EVENT.PLAYER_KICKED,a)}tool(a,b,c){this.send(GPProxy_.EVENT.TOOL,{t:this.state.turnNum,d:a,v:b},void 0,void 0,c)}undo(a,b){this.tool(2,a,b)}redo(a,b){this.tool(1,a,b)}ready(){this.send(GPProxy_.EVENT.READY,!this.state.user.ready)}getState(){return this.state}getScreen(){return GPProxy_.SCREEN_NAME[this.state.screen]??null}isRejoined(){return this.rejoinFlag}getUser(){return this.state.user}getNickname(){return this.state.user.nick}getUsername(){return this.formatNickname(this.getNickname())}formatNickname(a){return this.isDiscordAuth()?
...
this.s.lineSmoothingLevel&&this.lineSmoothingCounter%(this.s.lineSmoothingLevel+1)&&this.strokeCoords.push(this.getCanvasCoords(a));const b=this.buildStrokeData(this.strokeOptions,this.strokeCoords),c=this.buildSymmetryStrokesData(),d=this.isToolInTime(this.tool)?"stroke_ended":"stroke_started";this.dispatchEvent(new CustomEvent(d,{detail:{data:b,symmetryStrokesData:c}}));this.dispatchEvent(new CustomEvent("stroke",{detail:{data:b,symmetryStrokesData:c}}));this.isDrawing=!1;a.currentTarget.removeEventListener("pointermove",
this.strokeMove);this.strokeMove.cancel?.()}strokeProcess(){let a;this.isSymmetryEnabled&&(a=[],this.symmetryCoords.forEach((b,c)=>{a.push(this.buildStrokeData(this.symmetryStrokesOptions[c],b))}));this.dispatchEvent(new CustomEvent("stroke_process",{detail:{data:[...this.strokeOptions,...this.strokeCoords],symmetryStrokesData:a}}));!this.isSymmetryEnabled&&this.isToolInTime(this.tool)&&(clearTimeout(this.strokeTimer),this.strokeTimer=setTimeout(this.strokeProcess,1E3))}updateSymmetryStrokeCoords(a){a.forEach((b,
