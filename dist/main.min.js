(function(){
(()=>{const a=({pos:l,prevPos:m})=>l!==m,b=({minScale:l,maxScale:m,scale:p})=>p<=m&&p>=l,c=({minScale:l,maxScale:m,scale:p})=>({pos:n,prevPos:u,translate:q})=>b({minScale:l,maxScale:m,scale:p})&&a({pos:n,prevPos:u})?q+(n-u*p)*(1-1/p):q,d=({scale:l,minScale:m,maxScale:p,scaleSensitivity:n,deltaScale:u})=>[l,Math.max(m,Math.min(l+u/(n/l),p))],e=({scale:l,translateX:m,translateY:p})=>`matrix(${l}, 0, 0, ${l}, ${m}, ${p})`,g=({state:l,originX:m,originY:p})=>{l.transformation.translateX+=m;l.transformation.translateY+=
p;l.element.style.transform=e({scale:l.transformation.scale,translateX:l.transformation.translateX,translateY:l.transformation.translateY})},f=l=>({panBy:({originX:m,originY:p})=>g({state:l,originX:m,originY:p}),panTo:({originX:m,originY:p,scale:n})=>{l.transformation.scale=n;g({state:l,originX:m-l.transformation.translateX,originY:p-l.transformation.translateY})}}),h=l=>({zoom:({x:m,y:p,deltaScale:n,scaleSensitivity:u})=>{const {left:q,top:y}=l.element.getBoundingClientRect(),{minScale:z,maxScale:A,
defaultScaleSensitivity:r}=l,[x,t]=d({scale:l.transformation.scale,deltaScale:n,minScale:z,maxScale:A,scaleSensitivity:u||r});n=m-q;u=p-y;p=n/x;m=u/x;const w=c({scale:x,minScale:z,maxScale:A});n=w({pos:n,prevPos:l.transformation.originX,translate:l.transformation.translateX});u=w({pos:u,prevPos:l.transformation.originY,translate:l.transformation.translateY});l.element.style.transformOrigin=`${p/l.containerScale}px ${m/l.containerScale}px`;l.element.style.transform=e({scale:t,translateX:n/l.containerScale,
translateY:u/l.containerScale});l.transformation={originX:p,originY:m,translateX:n,translateY:u,scale:t}}}),k=l=>({getScale:()=>+l.transformation.scale,setScale:m=>{l.transformation.scale=+m},getScaleSensitivity:()=>+l.defaultScaleSensitivity,setScaleSensitivity:m=>{l.defaultScaleSensitivity=+m},setMinScale:m=>{l.minScale=+m},setMaxScale:m=>{l.maxScale=+m},getMinScale:()=>+l.minScale,getMaxScale:()=>+l.maxScale,setContainerScale:m=>{l.containerScale=+m},reset:()=>{g({state:l,originX:0-l.transformation.translateX,
...
Math.max(GPAbout_.HEADER_MIN_SCALE,this.getContainerScale());this.container.style.fontSize=`${16*a}px`;this.container.style.lineHeight=`${28*a}px`;this.updateUpdateNoticePos()}getContainerScale(){let a=(window.innerWidth-180)/1150;766*a>window.innerHeight&&(a=window.innerHeight/766);return a}shouldUpdateNoticeBeShown(){if(!this.updateNoticeFlag)return!1;const a=window.localStorage.getItem(GPAbout_.UPDATE_NOTICE_STORAGE)||"0.0";return this.isNewVersion(this.version,a,2)}isNewVersion(a,b,c=0){const [d,
e]=[a,b].map(g=>g.split(".").map(f=>+f));a=Math.max(d.length,e.length);a=c&&Math.min(a,c)||a;for(c=0;c<a;c++){b=d[c]??0;const g=e[c]??0;if(b!==g)return b>g}return!1}updateUpdateNoticePos(){if(this.notice){var {width:a}=this.versionElem.getBoundingClientRect(),{width:b}=this.notice.getBoundingClientRect();this.notice.style.left=`calc(-${Math.floor((b-a)/2)}px)`}}requestOutdatedScripts(){document.dispatchEvent(new Event("_get-outdated-scripts"))}printOutdatedScripts(a){a.size&&console.warn(`Outdated Scripts:\n${[...a.values()].map(b=>
`${b.name} (${b.version} \u2794 ${b.latestVersion}): ${b.url}?${Date.now()}.user.js`).join("\n")}`)}updateScript(a){document.dispatchEvent(new CustomEvent("_update-script",{detail:{name:a}}))}addNotice(a){a=Object.assign({},a,{id:++this.notificationId});this.notificationQueue.push(a);this.notificationQueue.sort((b,c)=>GPAbout_.NOTICE_PRIORITY[b.type]-GPAbout_.NOTICE_PRIORITY[c.type]);this.currentNotice&&GPAbout_.NOTICE_PRIORITY[this.currentNotice.type]===GPAbout_.NOTICE_PRIORITY[this.notificationQueue[0].type]||
this.showNotice(this.notificationQueue[0])}showNotice(a){this.currentNotice=a;this.notice?.remove();const {color:b,bg:c,bold:d,width:e}=GPAbout_.NOTICE_STYLES[a.type];this.container.style.setProperty("--notice-color",a.textColor??b);this.container.style.setProperty("--notice-background",a.bgColor??c);this.container.style.setProperty("--notice-font",a.bold??d?"Bold":"Regular");this.container.style.setProperty("--notice-width",`${a.width??e}px`);this.notice=document.createElement(a.url?"a":"div");this.notice.className=
"gp-update-notice_";this.notice.dataset.type=a.type;this.notice.innerHTML=this.buildNoticeMessage(a.text||(a.type===GPAbout_.NOTICE_TYPE.UPDATE?GPAbout_.UPDATE_MESSAGE:""));a.url&&(this.notice.href=a.url,this.notice.target="_blank",this.notice.title="\u041f\u0435\u0440\u0435\u0439\u0442\u0438 \u0432 Discord",this.notice.addEventListener("click",this.closeNotice));a.scripts&&(a=this.buildScripts(a.scripts),this.notice.appendChild(a));a=document.createElement("div");a.className="close-btn_";a.textContent=
"\u2716";a.title="";a.addEventListener("click",g=>{g.preventDefault();g.stopPropagation();this.closeNotice(g)});this.notice.appendChild(a);this.versionElem.appendChild(this.notice);this.updateUpdateNoticePos()}closeNotice(a){this.currentNotice.type===GPAbout_.NOTICE_TYPE.UPDATE&&window.localStorage.setItem(GPAbout_.UPDATE_NOTICE_STORAGE,this.version);this.notice.remove();this.notice=null;a=this.notificationQueue.indexOf(this.currentNotice);~a&&this.notificationQueue.splice(a,1);this.currentNotice=
null;this.showNextNotice()}showNextNotice(){this.notificationQueue.length&&this.showNotice(this.notificationQueue[0])}buildNoticeMessage(a){return a?a.split("\n").map(b=>`<div class="text">${b}</div>`).join(""):""}buildScripts(a){const b=document.createElement("div");b.className="scripts_";var c=document.createElement("div");c.className="title_";c.textContent="\u041e\u0431\u043d\u043e\u0432\u043b\u0435\u043d\u0438\u044f \u0441\u043a\u0440\u0438\u043f\u0442\u043e\u0432";b.appendChild(c);const d=document.createElement("div");
d.className="reload-row_";c=document.createElement("a");c.className="reload-btn_";c.href="/";c.textContent="\u041f\u0435\u0440\u0435\u0437\u0430\u0433\u0440\u0443\u0437\u0438\u0442\u044c \u0441\u0442\u0440\u0430\u043d\u0438\u0446\u0443";c.addEventListener("click",g=>{g.preventDefault();window.onbeforeunload=null;location.reload()});c.addEventListener("auxclick",g=>{g.preventDefault()});d.appendChild(c);const e=new Set;Array.from(a.values()).forEach((g,f)=>{var h=`${g.url}?${Date.now()}.user.js`;const k=
document.createElement("div");k.className="oudated-script_";var l=document.createElement("div");l.textContent=g.name;k.appendChild(l);l=document.createElement("div");l.textContent=`${g.version} \u2794 ${g.latestVersion}`;k.appendChild(l);g=document.createElement("div");k.appendChild(g);const m=document.createElement("a");m.className="script-update-link_";m.href=h;m.target="_blank";m.textContent="\u041e\u0431\u043d\u043e\u0432\u0438\u0442\u044c";h=p=>{e.add(f);e.size>=a.size&&d.classList.add("shown_");
...
(b/2*2-a/2))/2,a/2,b/2,0,0,a,b);this.focusCtx.globalCompositeOperation="destination-in";this.focusCtx.drawImage(d,0,0);this.focusCtx.restore()}clearViewfinderFocus(){this.focusCtx.clearRect(0,0,this.focus.width,this.focus.height)}setSource(a){const b=new Image;b.src=a;b.decode().then(()=>{this.zoomer.setImage(b);this.clearViewfinderFocus();this.up})}async imageCantHaveAlphaChannel(a){return GPAvatarEditor_.IMAGE_TYPES_WITHOUT_TRANSPARENCY.includes(a.type)?!0:!1}pngHasAlpha(a){a=new DataView(a);if(2303741511===
a.getUint32(0)&&218765834===a.getUint32(4))return a=a.getUint8(25),4===a||6===a}async handleFile(a){a.preventDefault();a=a.target.files||a.dataTransfer.files;if(a?.length&&(a=Array.from(a).find(b=>GPAvatarEditor_.IMAGE_TYPES.has(b.type)))){const b=await this.imageCantHaveAlphaChannel(a);b&&this.setForegroundState(!1);this.changeFgModeBtnState(b);const c=new FileReader;c.addEventListener("loadend",d=>{this.setSource(c.result)});c.readAsDataURL(a)}}setLivePreview(a){this.livePreviewEnabled=!!a}setBackground(a){this.backgroundColor=
a;this.isDragging||this.updateAvatarViews();this.dispatchEvent(new CustomEvent("background-color-changed",{detail:{backgroundColor:a}}))}setForegroundState(a){a!==this.hasForeground&&(this.hasForeground=a,this.container.classList.toggle("has-foreground",this.hasForeground),this.updateAvatarViews(),this.dispatchEvent(new CustomEvent("foreground-mode-changed",{detail:{foregroundMode:a}})))}setZoomSensitivity(a){this.zoomSensitivity=a;this.zoomer.setScaleFactor(a)}setImageSmoothing(a){this.imageSmoothingEnabled=
!!a;this.focusCtx.imageSmoothingEnabled=!!a;this.updateAvatarViews()}toggleForegroundMode(){this.setForegroundState(!this.hasForeground)}changeFgModeBtnState(a){this.fgModeBtn.classList.toggle("disabled",!!a)}mirror(){this.zoomer.mirror();this.updateAvatarViews()}requestAvatar(){return new Promise(a=>{this.buildAvatar().toBlob(a,GPAvatarEditor_.AVATAR_TYPE,GPAvatarEditor_.AVATAR_QUALITY)})}}window.GPAvatarEditor_=GPAvatarEditor_;class GPAvatars_ extends EventTarget{static URL="https://gpmod.github.io/avatars/{service}/{filename}";static NOTICE_COLORS={REVIEW:11726246,REMOVAL:12467260};static REQUEST_TYPE={REVIEW:"review",REMOVAL:"removal"};static DEFAULT_SETTINGS={zoomSensitivity:1.03,livePreview:!0,imageSmoothing:!0,hasForeground:!1,backgroundColor:"#cb55e2"};static SETTINGS_UI={editor:{type:"component",args:[this.getEditor]},reviewButtons:{type:"buttons",args:[this.getButtons]},_1:{size:"small"},livePreview:{type:"checkbox",
...
const f=a.scale;a.scale=function(u,q){c=c.scaleNonUniform(u,q);return f.call(a,u,q)};const h=a.rotate;a.rotate=function(u){c=c.rotate(180*u/Math.PI);return h.call(a,u)};const k=a.translate;a.translate=function(u,q){c=c.translate(u,q);return k.call(a,u,q)};const l=a.transform;a.transform=function(u,q,y,z,A,r){const x=b.createSVGMatrix();x.a=u;x.b=q;x.c=y;x.d=z;x.e=A;x.f=r;c=c.multiply(x);return l.call(a,u,q,y,z,A,r)};const m=a.setTransform;a.setTransform=function(u,q,y,z,A,r){c.a=u;c.b=q;c.c=y;c.d=
z;c.e=A;c.f=r;return m.call(a,u,q,y,z,A,r)};const p=a.resetTransform;a.resetTransform=function(){c=b.createSVGMatrix();d.length=0;return p.call(a)};const n=b.createSVGPoint();a.transformedPoint=function(u,q){n.x=u;n.y=q;return n.matrixTransform(c.inverse())}}setImage(a){this.image=a;this.reset()}setScaleFactor(a){this.scaleFactor=a}mirror(){this.image&&(this.ctx.translate(this.canvas.width,0),this.ctx.scale(-1,1),this.ctx.drawImage(this.image,0,0),this.redraw())}};class GPPlayersObserver_ extends EventTarget{static MONITOR_INTERVAL=200;static SCREEN={START:"start",LOBBY:"lobby",FIRST:"first",DRAW:"draw",WRITE:"write",REPLY:"reply",BOOK:"book",RESULT:"result"};static OBSERVED_SCREENS=new Set([this.SCREEN.LOBBY,this.SCREEN.BOOK,this.SCREEN.RESULT]);constructor(a){if(GPPlayersObserver_.instance)return GPPlayersObserver_.instance;super();GPPlayersObserver_.instance=this;this.gpproxy=a;this.isInitialized=!1;this.monitorTimer;this.inObserving=!1;this.observerConfig={childList:!0,
...
b({data:d,type:a}):c({error:"localization data not found",type:a})}).catch(d=>{c({error:d,type:a})}):c({error:"__NEXT_DATA__ is undefined",type:a})})}}window.GPProxy_=GPProxy_;class GPModulesManager_ extends EventTarget{static STORAGE_NAME="gp_mm";static FORCE_RELOAD=!0;static DEFAULT_PERMISSIONS=3685;static EDITION_STATE={FREE:"free",GIFT:"gift",SUBSCRIPTION:"subscription",TIME_LEFT:"time-left",EXPIRED:"expired"};static DEFAULT_SETTINGS={disabledList:[]};constructor(a){super();this.settings=JSON.parse(window.localStorage.getItem(GPModulesManager_.STORAGE_NAME))||GPModulesManager_.DEFAULT_SETTINGS;this.disabledList=new Set(this.settings.disabledList);this.modules=[];this.lm=
[];this.hiddenSettings=new Set;this.initialState={};this.rootThis;this.publicKey;this.pasteKey=null;this.settingsManager=new GPSettingsManager_(this);this.onMListClick=this.onMListClick.bind(this);this.onMListContextMenu=this.onMListContextMenu.bind(this);this.renderModulesList=this.renderModulesList.bind(this);this.renderError=this.renderError.bind(this);this.toggleSettingsVisibility=this.toggleSettingsVisibility.bind(this);this.addEventListener("disabled-list-updated",b=>{this.saveSettings()});
document.addEventListener("_auth_id",async({detail:{authId:b,authTypeId:c,isAuthorized:d}})=>{function e(l){this.lm=this.lm.filter(m=>1<<m.id&l?(m.disabled||(window[m.name].INIT.apply(this.rootThis,[this.root]),window[m.name].toggleSettingsVisibility=this.toggleSettingsVisibility(m),m.initialized=!0),m.locked=!1):m.locked=!0);this.renderModulesList()}async function g(l,m){m=(new TextEncoder).encode(m);var p=await crypto.subtle.digest("SHA-256",m);m=window.atob(l).slice(0,12);m={name:"AES-GCM",iv:new Uint8Array(Array.from(m).map(n=>
n.charCodeAt(0)))};p=await crypto.subtle.importKey("raw",p,m,!1,["decrypt"]);l=window.atob(l).slice(12);l=new Uint8Array(Array.from(l).map(n=>n.charCodeAt(0)));try{const n=await crypto.subtle.decrypt(m,p,l);return(new TextDecoder).decode(n)}catch(n){throw Error("Decrypt failed");}}function f(l){document.dispatchEvent(new CustomEvent("_auth_perms",{detail:{ps:l}}))}if(this.lm.length){var h=GPModulesManager_.DEFAULT_PERMISSIONS,k=null;try{if(!c)throw new GPAuthError_;if(!a||!this.publicKey)throw new GPAuthError_;
c=1===c?"t":"d";const l=JSON.parse(await g(a,this.publicKey)),m=JSON.parse(await g(l[c],b));l.e&&this.setEditionTitle(l.e,l.m);l.g&&(k=[GPModulesManager_.EDITION_STATE.GIFT]);if(m.ed){if(m.ed<Date.now())throw k=[GPModulesManager_.EDITION_STATE.EXPIRED],2&m.p&&(h|=16),new GPAuthError_;k=[GPModulesManager_.EDITION_STATE.TIME_LEFT,m.ed-Date.now()]}else l.g||(k=[GPModulesManager_.EDITION_STATE.SUBSCRIPTION]);k&&this.setEditionState(...k);e.call(this,h|m.p);f.call(this,h|m.p);this.renderModulesList()}catch(l){l instanceof
GPAuthError_||console.error(l),d&&!k&&(k=[GPModulesManager_.EDITION_STATE.FREE]),k&&this.setEditionState(...k),e.call(this,h),f.call(this,h)}}});document.addEventListener("_auth_key_changed",b=>{this.container.classList.add("reload-required_")});document.addEventListener("_auth_key",({detail:{publicKey:b}})=>{this.publicKey=b});document.addEventListener("_act_init",({detail:{pasteKey:b}})=>{this.pasteKey=b});this.render();this.setEditionTitle("base")}getElement(){return this.container}getSettingsManager(){return this.settingsManager}render(){this.container=
...
this.keyInput.className="key-input_ gp-input_ hidden";this.keyInput.placeholder="\u0421\u043a\u043e\u043f\u0438\u0440\u0443\u0439\u0442\u0435 \u043a\u043b\u044e\u0447 \u0441\u044e\u0434\u0430...";this.keyInput.spellcheck=!1;this.keyInput.autocomplete="off";this.keyInput.preventHiding=!1;this.keyInput.addEventListener("input",c=>{if(this.pasteKey){c=c.target.value.trim();var d=this.pasteKey(c);this.keyInput.classList.toggle("hidden",d);this.keyInput.classList.toggle("invalid_",c&&!d)}});this.keyInput.addEventListener("keydown",
c=>{switch(c.key){case "Escape":c.target.blur()}});this.keyInput.addEventListener("focus",c=>{document.dispatchEvent(new CustomEvent("_keys_lock",{detail:{state:!0}}));c.target.value="";c.target.preventHiding=!1});this.keyInput.addEventListener("blur",c=>{document.dispatchEvent(new CustomEvent("_keys_lock",{detail:{state:!1}}));c.target.value="";c.target.classList.remove("invalid_");c.target.preventHiding?c.target.preventHiding=!1:this.keyInput.classList.add("hidden")});this.container.appendChild(this.keyInput);
a=document.createElement("div");a.className="title_";a.textContent="\u041c\u043e\u0434\u0443\u043b\u0438";this.container.appendChild(a);this.edition=document.createElement("div");this.edition.className="edition_ hidden";this.edition.addEventListener("click",c=>{c=c.target.dataset.state;if(c===GPModulesManager_.EDITION_STATE.FREE||c===GPModulesManager_.EDITION_STATE.EXPIRED)c=document.createElement("a"),c.href="https://boosty.to/gpmod",c.target="_blank",c.click()});this.container.appendChild(this.edition);
this.editionTitle=document.createElement("div");this.editionTitle.className="edition-title_";this.edition.appendChild(this.editionTitle);a=document.createElement("div");a.className="ribbon_";this.edition.appendChild(a);this.modulesList=document.createElement("div");this.modulesList.className="modules-list_";this.modulesList.addEventListener("contextmenu",this.onMListContextMenu);this.modulesList.addEventListener("pointerup",this.onMListClick);this.container.appendChild(this.modulesList);this.reloadBtn=
document.createElement("div");this.reloadBtn.className="gp-btn_ reload-btn_";this.reloadBtn.classList.add("disabled_");this.reloadBtn.textContent="\u041f\u0435\u0440\u0435\u0437\u0430\u0433\u0440\u0443\u0437\u0438\u0442\u044c";this.reloadBtn.addEventListener("click",this.reloadBtnClickHandler);this.container.appendChild(this.reloadBtn);document.body.appendChild(this.container)}setEditionTitle(a,b=!1){this.editionTitle.textContent=`${a} ed.`;this.edition.classList.toggle("demo_",!!b);this.edition.classList.remove("hidden")}setEditionState(a,
b){this.edition.dataset.state=a;this.edition.title="";a===GPModulesManager_.EDITION_STATE.TIME_LEFT?this.edition.dataset.timeLeft=this.formatTimeLeft(b):a===GPModulesManager_.EDITION_STATE.EXPIRED&&(this.edition.title="\u041e\u0444\u043e\u0440\u043c\u0438\u0442\u044c \u043f\u043e\u0434\u043f\u0438\u0441\u043a\u0443 \u043d\u0430 \u043c\u043e\u0434 \u043d\u0430 Boosty")}formatTimeLeft(a){a=Math.trunc(a/864E5);const b=a%100;var c=b%10;let d;if(10<=b&&20>b)d=2;else if(1===c)d=0;else if(1<c&&5>c)d=1;else if(!c||
4<c)d=2;c=["\u041e\u0441\u0442\u0430\u043b\u0441\u044f","\u041e\u0441\u0442\u0430\u043b\u043e\u0441\u044c"];const e=["\u0434\u0435\u043d\u044c","\u0434\u043d\u044f","\u0434\u043d\u0435\u0439"];return`${d?c[1]:c[0]} ${1>b?"\u043c\u0435\u043d\u044c\u0448\u0435 \u0434\u043d\u044f":`${a} ${e[d]}`}`}onMListClick(a){2===a.button?a.preventDefault():a.target.matches(".module-title_, .module-settings_")&&!a.target.classList.contains("disabled_")&&this.settingsManager.toggle(!0,a.target.dataset.module,a.ctrlKey||
1===a.button)}onMListContextMenu(a){a.preventDefault();a.stopPropagation();a.target.matches(".module-title_")&&!a.target.classList.contains("disabled_")&&this.settingsManager.toggle(!0,a.target.dataset.module,a.ctrlKey||1===a.button)}compareToInitialState(){const a=[...this.modulesList.querySelectorAll(".gp-cb_")].some(b=>b.checked!==this.initialState[b.name]);this.reloadBtn.classList.toggle("alert_",a);this.reloadBtn.classList.toggle("disabled_",!a)}reloadBtnClickHandler(a){GPModulesManager_.FORCE_RELOAD&&
...
h.addEventListener("click",()=>{this.settingsView.classList.toggle("json-data_");k.focus()});h.addEventListener("animationend",p=>{p.target.classList.remove("failure_");p.target.classList.remove("success_")});f.appendChild(h);var k=document.createElement("input");k.className="json-data-input_ gp-input_";k.placeholder="\u0421\u043a\u043e\u043f\u0438\u0440\u0443\u0439\u0442\u0435 \u0441\u044e\u0434\u0430 \u0434\u0430\u043d\u043d\u044b\u0435...";k.spellcheck=!1;k.autocomplete="off";k.addEventListener("input",
p=>{this.jsonDataInputHandler(a,p.target,h)});k.addEventListener("focus",p=>{document.dispatchEvent(new CustomEvent("_keys_lock",{detail:{state:!0}}))});k.addEventListener("blur",p=>{document.dispatchEvent(new CustomEvent("_keys_lock",{detail:{state:!1}}))});f.appendChild(k);var l=document.createElement("div");l.className="restore-btn_ btn_ gp-btn_";l.textContent="\u0412\u043e\u0441\u0441\u0442\u0430\u043d\u043e\u0432\u0438\u0442\u044c";l.title="\u0412\u0435\u0440\u043d\u0443\u0442\u044c\u0441\u044f \u043a \u043d\u0430\u0441\u0442\u0440\u043e\u0439\u043a\u0430\u043c \u043f\u043e\u0441\u043b\u0435\u0434\u043d\u0435\u0433\u043e \u0437\u0430\u043f\u0443\u0441\u043a\u0430 \u0438\u0433\u0440\u044b";
...
f=document.createDocumentFragment();for(let p in e){l=e[p]||{};const {type:n,args:u,hidden:q,description:y,text:z}=l;if(this.isLabel(p)){var m=document.createElement("div");m.className=z?"group-label_":"separator_";m.classList.toggle(l.size,!!l.size);m.textContent=z;f.appendChild(m)}else if(n&&!q)if("component"===n)[l]=u,m=document.createElement("div"),m.className="component_",m.appendChild(l()),f.appendChild(m);else if("buttons"===n){const A=document.createElement("div");A.className="buttons_";[l]=
u;l().forEach(r=>{r.classList.add("btn_");A.appendChild(r)});f.appendChild(A)}else if("buttons-g"===n){const A=document.createElement("div");A.className="buttons_";u.forEach(([r,x,t])=>{const w=document.createElement("div");w.className="btn_";w.title=x;w.textContent=r;w.addEventListener("click",t);A.appendChild(w)});f.appendChild(A)}else l=this.buildTooltip(y),f.appendChild(l),l=this.buildLabel(p,y),f.appendChild(l),l=this.buildControl(e,p,d[p],b,c,d),f.appendChild(l)}g.appendChild(f);this.controls=
...
d.style.cssText=`top: ${g-(c-h)/2}px; left: ${e+f+12}px`;window.gpSettingsTooltip=d});b.addEventListener("pointerleave",c=>{window.gpSettingsTooltip&&(window.gpSettingsTooltip.remove(),window.gpSettingsTooltip=null)});return b}buildLabel(a,b){const c=document.createElement("div");c.className="label_";c.title=b||"";c.textContent=a.split(/(?<![A-Z])(?=[A-Z])|(?=[A-Z]\B)/).join(" ");return c}buildControl(a,b,c,d,e,g){const {type:f,data:h,args:k,formatter:l,proxyProp:m,inputTrigger:p}=a[b];a=document.createElement("div");
a.className="control_";let n;switch(f){case "range":const q=document.createElement("div");q.className="preview_ gp-range-preview_";q.textContent=l?l(c):c;const [y,z,A,r]=k;n=document.createElement("input");n.className="gp-range_";n.type="range";n.min=y;n.max=z;n.step=A;n.value=c;n.name=b;r&&(n.style.direction="rtl");n.addEventListener("input",t=>{q.textContent=l?l(Number.parseFloat(t.target.value)):t.target.value});a.appendChild(n);a.appendChild(q);break;case "list":n=document.createElement("select");
n.className="gp-list_";n.autocomplete="off";n.name=b;(m&&g[m]||(k.length?k:[c])).forEach((t,w)=>{const v=document.createElement("option");v.value=t;v.textContent=l?l(t,w):t;n.appendChild(v)});n.value=c;a.appendChild(n);break;case "list-m":case "list-md":var u="list-m"===f?k:c;n=document.createElement("form");n.className="gp-list-m_";n.name=b;u.forEach((t,w)=>{var v=document.createElement("input");v.id=`ctrl-${b}-${w}_`;v.type="checkbox";v.value=t;v.name=b;v.checked=c.includes(t);n.appendChild(v);
v=document.createElement("label");v.textContent=l?l(t,w):t;v.setAttribute("for",`ctrl-${b}-${w}_`);n.appendChild(v)});a.appendChild(n);break;case "list-i":const x=[];n=document.createElement("form");n.className="gp-list-i_";n.name=b;Object.keys(h.icons).forEach((t,w)=>{const v=document.createElement("input");v.id=`ctrl-${b}-${w}_`;v.type="checkbox";v.name=b;v.value=t;v.checked=c===t;v.addEventListener("click",C=>{x.forEach(D=>{D.checked=D===v})});n.appendChild(v);x.push(v);const B=document.createElement("label");
B.className="gp-list-i-icon_";B.title=t.split("-").map(C=>C[0].toUpperCase()+C.slice(1)).join(" ");B.style.backgroundImage=`url("${h.icons[t]}")`;B.setAttribute("for",`ctrl-${b}-${w}_`);n.appendChild(B)});a.appendChild(n);break;case "checkbox":n=document.createElement("div");n.className="gp-tumbler_";u=document.createElement("input");u.id=`ctrl-${b}_`;u.type="checkbox";u.name=b;u.checked=c;n.appendChild(u);u=document.createElement("label");u.setAttribute("for",`ctrl-${b}_`);n.appendChild(u);a.appendChild(n);
break;case "color":n=document.createElement("input");n.className="gp-color_";n.type="color";n.value=c;n.name=b;a.appendChild(n);break;case "input":({hidden:u}=k||{}),n=document.createElement("input"),n.className="gp-input_",n.classList.toggle("secure-input_",!!u),n.spellcheck=!1,n.autocomplete="off",n.value=c,n.name=b,n.addEventListener("keydown",t=>{switch(t.key){case "Escape":t.target.value=t.target.dataset.initialValue;case "Enter":t.target.blur()}}),n.addEventListener("focus",t=>{t.target.select();
document.dispatchEvent(new CustomEvent("_keys_lock",{detail:{state:!0}}));t.target.dataset.initialValue=t.target.value}),n.addEventListener("blur",t=>{window.getSelection().removeAllRanges();document.dispatchEvent(new CustomEvent("_keys_lock",{detail:{state:!1}}));delete t.target.dataset}),a.appendChild(n)}n.addEventListener(p?"input":"change",q=>{q={[q.target.name]:this.parseValue(q.target,f)};this.dispatchEvent(new CustomEvent("update",{detail:{source:d,storage:e,settings:Object.assign(g,q),changedSettings:q}}))});
...
hand:[["Space"]],clear:[["KeyF"]],brushTool:[["KeyB"]],eraserTool:[["KeyE"]],save:[["ControlLeft","KeyS"]],undo:[["ControlLeft","KeyZ"]],redo:[["ControlLeft","KeyY"],["ControlLeft","ShiftLeft","KeyZ"]],save:[["ControlLeft","KeyS"]]};static NATIVE_BINDINGS_KEYS=new Set(["KeyE","KeyB","KeyZ"]);static MODULE={title:"Painter",dependencies:["GPDrawInterface_","GPColorUtils_","GPColorPicker_"]};static INIT(a){new GPPainter_(a.gpproxy,a.timer,a.reference,a.ac)}constructor(a,b,c,d){super();
...
clear:{handler:this.clear,description:"\u041e\u0447\u0438\u0441\u0442\u043a\u0430 \u0445\u043e\u043b\u0441\u0442\u0430 (\u043d\u0435 \u0440\u0430\u0431\u043e\u0442\u0430\u0435\u0442 \u0432 \u0440\u0435\u0436\u0438\u043c\u0430\u0445 \u0410\u043d\u0438\u043c\u0430\u0446\u0438\u044f, \u0424\u043e\u043d \u0438 \u0421\u043e\u043b\u043e)"},brushTool:{handler:this.brushTool,description:"\u0418\u043d\u0441\u0442\u0440\u0443\u043c\u0435\u043d\u0442 \u041a\u0438\u0441\u0442\u044c"},eraserTool:{handler:this.eraserTool,description:"\u0418\u043d\u0441\u0442\u0440\u0443\u043c\u0435\u043d\u0442 \u041b\u0430\u0441\u0442\u0438\u043a"},undo:{handler:this.undo,repeat:!0,description:"\u041e\u0442\u043c\u0435\u043d\u0430 \u043f\u043e\u0441\u043b\u0435\u0434\u043d\u0435\u0433\u043e \u0448\u0442\u0440\u0438\u0445\u0430"},redo:{handler:this.redo,repeat:!0,
...
[];this.history=[];this.undos=[];this.lastDrawData=this.backgroundData=null;this.lastDelimiterIndex=-1;this.isSigned=!1;this.mStrokeId=null;this.mStrokeOrigin=this.getRandomInt(GPPainter_.M_STROKE_RANGE);this.isSubscribed=!1;this.pointerY=this.pointerX=0;this.isSymmetryGuidesMoved=this.isSymmetryGuidesEditable=this.isSymmetryEnabled=this.isDrawing=this.isZoomedToPoint=this.isHand=this.isEraser=this.isMirrored=this.isZooming=this.isBrightnessSetting=this.isThicknessSetting=this.isOpacitySetting=this.isDropperDown=this.isDropper=
...
this.gpproxy.getState(),b=this.gpproxy.getCurrentTurnId();this.searchTimer=null;this.strokeOptions=[];this.strokeCoords=[];this.history=a.draw||[];this.backgroundData=this.ac?.isSuspicious(`tbg_${b}`)?[]:a.background;this.lastDrawData=this.ac?.isSuspicious(`t_${b}`)?[]:a.lastDraw;this.lastDelimiterIndex=this.history.findLastIndex(c=>11===c[0]);this.strokeId=this.calculateStrokeID();this.undos=[];this.pointerY=this.pointerX=0;this.isDrawing=this.isZoomedToPoint=this.isHand=this.isEraser=this.isMirrored=this.isZooming=this.isBrightnessSetting=this.isThicknessSetting=
...
{detail:{data:a}}));this.dispatchEvent(new CustomEvent("stroke",{detail:{data:a}}))}brushTool(){this.setTool(GPPainter_.TOOL.BRUSH);this.iface.updateToolbar(this.tool);this.updatePointer()}eraserTool(){this.setTool(GPPainter_.TOOL.ERASER);this.iface.updateToolbar(this.tool);this.updatePointer()}undo(){let a;do{if(!this.history.length||11===this.history.at(-1)[0])return;a=this.history.pop();this.undos.push(a);this.dispatchEvent(new CustomEvent("stroke_undo",{detail:{data:a}}))}while(void 0!==this.symmetryStrokesGroups[a[1]]&&
...
!0}enable(){this.container.classList.remove("disabled_");this.isDisabled=!1}drawFunction(a,b,c=this.density,d=!0,e=!0){a.lineCap=a.lineJoin="round";a.save();if(b){let p=[];for(var g=0;g<b.length;g++){var f=b[g],h=f[0];f=(f[1],f.slice(2));if(10!=h){var k=void 0;a.save();if(8!=h){for(k=1;k<f.length;k++)f[k]=[f[k][0]*c,f[k][1]*c];if(2<f.length)switch(h){case 1:h=f[0];a.strokeStyle=h[0];a.lineWidth=h[1];a.globalAlpha=h[2];a.lineWidth*=c;k=[f[1][0],f[1][1]];a.beginPath();a.moveTo(...k);for(var l=2;l<f.length;l++)k=
...
c.drawImage(this.background,0,0,d.width,d.height);c.drawImage(this.lastDraw,0,0,d.width,d.height);c.drawImage(this.canvas,0,0,d.width,d.height)}else c=this.canvasCtx;d=this.getPixelColor(a,b);return this.getFillRects(c,a,b,d,this.canvas.width,this.canvas.height).map((e,g)=>4>g%5?Math.round(e/this.density):e)}getFillRects(a,b,c,d,e,g){var f=function(t,w,u){var x=w,v=u;if(m(w,u,t)){for(;m(x+1,v,t);)x++;var B=
  x;do for(x=w-1,v++;m(x+1,v,t)&&x+1<=B;)x++;while(x==B);return{x:w,y:u,w:B-w,h:--v-u}}return{w:-1,h:-1}},h=function(t,w,u){var x=w,v=u;if(m(w,u,t)){for(;m(x-1,v,t);)x--;var B=x;do for(x=w+1,v--;m(x-1,v,t)&&x-1>=B;)x--;while(x==B);return{x:B,y:++v,w:w-B,h:u-v}}return{w:-1,h:-1}},k=function(t,w,u){var x=w,v=u;if(m(w,u,t)){for(;m(x,v+1,t);)v++;var B=v;do for(v=u-1,x--;m(x,v+1,t)&&v+1<=B;)v++;while(v==B);return{x:++x,y:u,w:w-x,h:B-u}}return{w:-1,h:-1}},l=function(t,w,u){var x=w,v=u;if(m(w,u,t)){for(;m(x,
  v-1,t);)v--;var B=v;do for(v=u+1,x++;m(x,v-1,t)&&v-1>=B;)v--;while(v==B);return{x:w,y:B,w:--x-w,h:u-B}}return{w:-1,h:-1}},m=function(t,w,u){if(p[t][w])return!1;t=4*(t+w*e);t=r.slice(t,t+4);w=Math.abs(u[1]-t[1]);var x=Math.abs(u[2]-t[2]),v=Math.abs(u[3]-t[3]);return 20>Math.abs(u[0]-t[0])&&20>w&&20>x&&20>v||0==t[3]&&255==u[0]&&255==u[1]&&255==u[2]||0==u[3]&&255==t[0]&&255==t[1]&&255==t[2]},n=function(t){for(var w=t[0],u=t[1],x=w+t[2],v=u+t[3];w<x;w++)for(var B=u;B<v;B++)p[w][B]=!0;return t};b=Math.round(b);
  c=Math.round(c);var p;parseInt(d.replace("#","0x"));!function(){p=[];for(var t=-1;t<=e;t++)p[t]=[];p[-1]=[];p[e]=[];for(t=-1;t<=g;t++)p[-1][t]=1,p[e][t]=1;for(t=0;t<e;t++)p[t][-1]=1,p[t][g]=1}();var r=a.getImageData(0,0,e,g).data;a=4*(b+c*e);a=[r[a],r[a+1],r[a+2],r[a+3]];d=[];for(var q=0;q<=e;q++)d[q]=[];for(;m(b-1,c,a);)b--;for(;m(b,c-1,a);)c--;var z;q=f(a,b,c);c={x:b,y:c,w:q.w,h:q.h,ref:0,andada:0,stack:0};var A=q.w,y=[].concat(n([q.x,q.y,q.w+1,q.h+1]),[0]);do{b=0;for(2==c.ref&&(b+=c.andada);b<=
  c.h;)-1!=(z=(q=l(a,c.x+c.w+1,c.y+c.h-b)).h)?(d[z].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:1,andada:c.h+1-b,stack:y.length}),y.push.apply(y,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),z>A&&(A=z),b+=z):b++;b=0;for(1==c.ref&&(b+=c.andada);b<=c.h;)-1!=(z=(q=k(a,c.x-1,c.y+b)).h)?(d[z].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:2,andada:c.h+1-b,stack:y.length}),y.push.apply(y,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),z>A&&(A=z),b+=z):b++;b=0;for(4==c.ref&&(b+=c.andada);b<=c.w;)-1!=(z=(q=f(a,c.x+b,c.y+c.h+1)).w)?(d[z].push({x:q.x,
  y:q.y,w:q.w,h:q.h,ref:3,andada:c.w+1-b,stack:y.length}),y.push.apply(y,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),z>A&&(A=z),b+=z):b++;b=0;for(3==c.ref&&(b+=c.andada);b<=c.w;)-1!=(z=(q=h(a,c.x+c.w-b,c.y-1)).w)?(d[z].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:4,andada:c.w+1-b,stack:y.length}),y.push.apply(y,n([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),z>A&&(A=z),b+=z):b++;for(c=d[A].pop();null==c&&0<A;)c=d[--A].pop()}while(null!=c);return y}setTimer(){this.timer&&(this.timer.reset(),this.timer.settings.runAfterTurnStart&&this.gpproxy.isGameTime(GPProxy_.GAME_TIME.HOSTS_DECISION)&&this.gpproxy.isOwner()&&this.timer.start())}stopTimer(){this.timer&&this.timer.reset()}sign(){const a=this.buildMStroke();this.gpproxy.tool(1,a);this.gpproxy.tool(3,a);this.mStrokeId=a[1];
...
GPPainter_.M_STROKE_OFFSET,b[c+1]+GPPainter_.ORIGINAL_HEIGHT+GPPainter_.M_STROKE_OFFSET]);return a}formatUsername(a){return this.gpproxy.isDiscordAuth()?a:a.toLowerCase()}shouldBeSigned(){return!this.isSigned&&this.gpproxy.isAuthorized&&this.history.length-(this.lastDelimiterIndex+1)>this.mStrokeOrigin}getRandomInt(a){return Math.floor(Math.random()*(a+1))}saveSettings(){window.localStorage.setItem("gp_painter",JSON.stringify(this.settings))}getSettings(){return Object.assign({},this.settings)}getDefaultSettings(){return Object.assign({},
...
this.painter.changeSymmetryRadialGuidesCount(h.deltaY):this.painter.changeSymmetryMandalaGuidesCount(h.deltaY)});this.symmetryGuidesCount.addEventListener("pointerdown",h=>{let k=h.clientY;const l=m=>{const p=m.clientY-k;Math.abs(p)>=GPPainter_.SYMMETRY_GUIDES_COUNT_SENSITIVITY&&(m.currentTarget.dataset.count=this.painter.getSymmetryMode()===GPPainter_.SYMMETRY_MODES.RADIAL?this.painter.changeSymmetryRadialGuidesCount(p):this.painter.changeSymmetryMandalaGuidesCount(p),k=m.clientY)};this.symmetryGuidesCount.setPointerCapture(h.pointerId);
...
c=document.createDocumentFragment();d=this.painter.getUISettings();for(let e in d){const {type:g,args:f,description:h,text:k,inputTrigger:l}=this.painter.getUISetting(e)||{};if(e.startsWith("_")){d=document.createElement("div");d.className="group-label_";d.textContent=k;c.appendChild(d);continue}if(!g)continue;d=document.createElement("div");d.className="label_";d.title=h;d.textContent=e.split(/(?=[A-Z])/).join(" ");let m=document.createElement("div");m.className="control_";let p;switch(g){case "range":const n=
document.createElement("div");n.className="preview_ gp-range-preview_";n.textContent=a[e];const [u,q,y]=f;p=document.createElement("input");p.className="gp-range_";p.type=g;p.min=u;p.max=q;p.step=y;p.value=a[e];p.name=e;p.addEventListener("input",r=>{n.textContent=r.target.value});m.appendChild(p);m.appendChild(n);break;case "list":p=document.createElement("select");p.className="gp-list_";p.name=e;f.forEach(r=>{const x=document.createElement("option");x.value=r;x.textContent=r;p.appendChild(x)});
p.value=a[e];m.appendChild(p);break;case "checkbox":p=document.createElement("div");p.className="gp-tumbler_";const z=document.createElement("input");z.id=`ctrl-${e}_`;z.type="checkbox";z.name=e;z.checked=a[e];p.appendChild(z);const A=document.createElement("label");A.for=`ctrl-${e}_`;A.addEventListener("click",r=>{z.click()});p.appendChild(A);m.appendChild(p);break;case "color":p=document.createElement("input"),p.className="gp-color_",p.type=g,p.value=a[e],p.name=e,m.appendChild(p)}p.addEventListener(l?
"input":"change",n=>{this.dispatchEvent(new CustomEvent("settings_updated",{detail:{settings:{[n.target.name]:this.parseSettingsValue(n.target)}}}))});c.appendChild(d);c.appendChild(m)}b.appendChild(c);this.toggleScrollbar(b);b.scrollTop=this.settingsScrollPos}getInitialSettings(){return Object.assign({},this.initialSettings)}parseSettingsValue(a){switch(a.type){case "range":return Number.parseFloat(a.value);case "checkbox":return a.checked;default:return a.value}}resetSettings(a){a=this.painter.getDefaultSettings();
...
var d=document.createElement("div");d.className="reset-btn_ btn_ gp-btn_";d.textContent="\u0421\u0431\u0440\u043e\u0441\u0438\u0442\u044c \u043d\u0430\u0441\u0442\u0440\u043e\u0439\u043a\u0438";d.title="\u0412\u044b\u0441\u0442\u0430\u0432\u0438\u0442\u044c \u043d\u0430\u0441\u0442\u0440\u043e\u0439\u043a\u0438 \u043f\u043e \u0443\u043c\u043e\u043b\u0447\u0430\u043d\u0438\u044e";d.addEventListener("click",this.resetBindings);c.appendChild(d);c=document.createDocumentFragment();for(let e in a){if(!this.painter.tools[e])continue;d=document.createElement("div");
