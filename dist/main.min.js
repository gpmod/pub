...
this.canvas.width;e.height=this.canvas.height;d=e.getContext("2d");d.drawImage(this.background,0,0,e.width,e.height);d.drawImage(this.lastDraw,0,0,e.width,e.height);d.drawImage(this.canvas,0,0,e.width,e.height)}else d=this.canvasCtx;return this.getFillRects(d,a,b,c,this.canvas.width,this.canvas.height).map((e,f)=>4>f%5?Math.round(e/this.density):e)}getFillRects(a,b,c,d,e,f){var g=function(r,D,B){var A=D,w=B;if(m(D,B,r)){for(;m(A+1,w,r);)A++;var F=A;do for(A=D-1,w++;m(A+1,w,r)&&A+1<=F;)A++;while(A==
F);return{x:D,y:B,w:F-D,h:--w-B}}return{w:-1,h:-1}},k=function(r,D,B){var A=D,w=B;if(m(D,B,r)){for(;m(A-1,w,r);)A--;var F=A;do for(A=D+1,w--;m(A-1,w,r)&&A-1>=F;)A--;while(A==F);return{x:F,y:++w,w:D-F,h:B-w}}return{w:-1,h:-1}},h=function(r,D,B){var A=D,w=B;if(m(D,B,r)){for(;m(A,w+1,r);)w++;var F=w;do for(w=B-1,A--;m(A,w+1,r)&&w+1<=F;)w++;while(w==F);return{x:++A,y:B,w:D-A,h:F-B}}return{w:-1,h:-1}},l=function(r,D,B){var A=D,w=B;if(m(D,B,r)){for(;m(A,w-1,r);)w--;var F=w;do for(w=B+1,A++;m(A,w-1,r)&&
w-1>=F;)w--;while(w==F);return{x:D,y:F,w:--A-D,h:B-F}}return{w:-1,h:-1}},m=function(r,D,B){if(t[r][D])return!1;r=4*(r+D*e);r=x.slice(r,r+4);D=Math.abs(B[1]-r[1]);var A=Math.abs(B[2]-r[2]),w=Math.abs(B[3]-r[3]);return 20>Math.abs(B[0]-r[0])&&20>D&&20>A&&20>w||0==r[3]&&255==B[0]&&255==B[1]&&255==B[2]||0==B[3]&&255==r[0]&&255==r[1]&&255==r[2]},p=function(r){for(var D=r[0],B=r[1],A=D+r[2],w=B+r[3];D<A;D++)for(var F=B;F<w;F++)t[D][F]=!0;return r};b=Math.round(b);c=Math.round(c);var t;d=parseInt(d.replace("#",
"0x"));var q=d%256,n=(d=Math.floor(d/256))%256,u=d=Math.floor(d/256);!function(){t=[];for(var r=-1;r<=e;r++)t[r]=[];t[-1]=[];t[e]=[];for(r=-1;r<=f;r++)t[-1][r]=1,t[e][r]=1;for(r=0;r<e;r++)t[r][-1]=1,t[r][f]=1}();var x=a.getImageData(0,0,e,f).data;a=4*(b+c*e);a=[x[a],x[a+1],x[a+2],x[a+3]];d=[];for(var y=0;y<=e;y++)d[y]=[];if(m(b,c,[u,n,q,255]))return[];for(;m(b-1,c,a);)b--;for(;m(b,c-1,a);)c--;var v;q=g(a,b,c);c={x:b,y:c,w:q.w,h:q.h,ref:0,andada:0,stack:0};n=q.w;u=[].concat(p([q.x,q.y,q.w+1,q.h+1]),
[0]);do{b=0;for(2==c.ref&&(b+=c.andada);b<=c.h;)-1!=(v=(q=l(a,c.x+c.w+1,c.y+c.h-b)).h)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:1,andada:c.h+1-b,stack:u.length}),u.push.apply(u,p([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),v>n&&(n=v),b+=v):b++;b=0;for(1==c.ref&&(b+=c.andada);b<=c.h;)-1!=(v=(q=h(a,c.x-1,c.y+b)).h)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:2,andada:c.h+1-b,stack:u.length}),u.push.apply(u,p([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),v>n&&(n=v),b+=v):b++;b=0;for(4==c.ref&&(b+=c.andada);b<=
c.w;)-1!=(v=(q=g(a,c.x+b,c.y+c.h+1)).w)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:3,andada:c.w+1-b,stack:u.length}),u.push.apply(u,p([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),v>n&&(n=v),b+=v):b++;b=0;for(3==c.ref&&(b+=c.andada);b<=c.w;)-1!=(v=(q=k(a,c.x+c.w-b,c.y-1)).w)?(d[v].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:4,andada:c.w+1-b,stack:u.length}),u.push.apply(u,p([q.x,q.y,q.w+1,q.h+1]).concat([c.stack])),v>n&&(n=v),b+=v):b++;for(c=d[n].pop();null==c&&0<n;)c=d[--n].pop()}while(null!=c);return u}setTimer(){this.timer&&
