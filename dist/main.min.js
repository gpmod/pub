...
c.drawImage(this.background,0,0,d.width,d.height);c.drawImage(this.lastDraw,0,0,d.width,d.height);c.drawImage(this.canvas,0,0,d.width,d.height)}else c=this.canvasCtx;d=this.getPixelColor(a,b);return this.getFillRects(c,a,b,d,this.canvas.width,this.canvas.height).map(e=>Math.round(e/this.density))}getFillRects(a,b,c,d,e,g){let f=function(r,x,t){let w=x,v=t;if(m(x,t,r)){for(;m(w+1,v,r);)w++;let B=w;do for(w=x-1,v++;m(w+1,v,r)&&w+1<=B;)w++;while(w==B);return{x,y:t,w:B-x,h:--v-t}}return{w:-1,h:-1}},h=
function(r,x,t){let w=x,v=t;if(m(x,t,r)){for(;m(w-1,v,r);)w--;let B=w;do for(w=x+1,v--;m(w-1,v,r)&&w-1>=B;)w--;while(w==B);return{x:B,y:++v,w:x-B,h:t-v}}return{w:-1,h:-1}},k=function(r,x,t){let w=x,v=t;if(m(x,t,r)){for(;m(w,v+1,r);)v++;let B=v;do for(v=t-1,w--;m(w,v+1,r)&&v+1<=B;)v++;while(v==B);return{x:++w,y:t,w:x-w,h:B-t}}return{w:-1,h:-1}},l=function(r,x,t){let w=x,v=t;if(m(x,t,r)){for(;m(w,v-1,r);)v--;let B=v;do for(v=t+1,w++;m(w,v-1,r)&&v-1>=B;)v--;while(v==B);return{x,y:B,w:--w-x,h:t-B}}return{w:-1,
h:-1}},m=function(r,x,t){if(n[r][x])return!1;r=4*(r+x*e);r=u.slice(r,r+4);x=Math.abs(t[1]-r[1]);let w=Math.abs(t[2]-r[2]),v=Math.abs(t[3]-r[3]);return 20>Math.abs(t[0]-r[0])&&20>x&&20>w&&20>v||0==r[3]&&255==t[0]&&255==t[1]&&255==t[2]||0==t[3]&&255==r[0]&&255==r[1]&&255==r[2]},p=function(r){for(let x=r[0],t=r[1],w=x+r[2],v=t+r[3],B=x;B<w;B++)for(let C=t;C<v;C++)n[B][C]=!0;return r};b=Math.round(b);c=Math.round(c);let n;parseInt(d.replace("#","0x"));!function(){n=[];for(var r=-1;r<=e;r++)n[r]=[];n[-1]=
[];n[e]=[];for(r=-1;r<=g;r++)n[-1][r]=1,n[e][r]=1;for(r=0;r<e;r++)n[r][-1]=1,n[r][g]=1}();var u=a.getImageData(0,0,e,g).data;a=4*(b+c*e);a=[u[a],u[a+1],u[a+2],u[a+3]];d=[];for(var q=0;q<=e;q++)d[q]=[];for(;m(b-1,c,a);)b--;for(;m(b,c-1,a);)c--;let y;q=f(a,b,c);c={x:b,y:c,w:q.w,h:q.h,ref:0,next:0};let z=q.w,A=p([q.x,q.y,q.w+1,q.h+1]);do{b=0;for(2==c.ref&&(b+=c.next);b<=c.h;)-1!=(y=(q=l(a,c.x+c.w+1,c.y+c.h-b)).h)?(d[y].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:1,next:c.h+1-b}),A.push.apply(A,p([q.x,q.y,q.w+
1,q.h+1])),y>z&&(z=y),b+=y):b++;b=0;for(1==c.ref&&(b+=c.next);b<=c.h;)-1!=(y=(q=k(a,c.x-1,c.y+b)).h)?(d[y].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:2,next:c.h+1-b}),A.push.apply(A,p([q.x,q.y,q.w+1,q.h+1])),y>z&&(z=y),b+=y):b++;b=0;for(4==c.ref&&(b+=c.next);b<=c.w;)-1!=(y=(q=f(a,c.x+b,c.y+c.h+1)).w)?(d[y].push({x:q.x,y:q.y,w:q.w,h:q.h,ref:3,next:c.w+1-b}),A.push.apply(A,p([q.x,q.y,q.w+1,q.h+1])),y>z&&(z=y),b+=y):b++;b=0;for(3==c.ref&&(b+=c.next);b<=c.w;)-1!=(y=(q=h(a,c.x+c.w-b,c.y-1)).w)?(d[y].push({x:q.x,
y:q.y,w:q.w,h:q.h,ref:4,next:c.w+1-b}),A.push.apply(A,p([q.x,q.y,q.w+1,q.h+1])),y>z&&(z=y),b+=y):b++;for(c=d[z].pop();null==c&&0<z;)c=d[--z].pop()}while(null!=c);return A}setTimer(){this.timer&&(this.timer.reset(),this.timer.settings.runAfterTurnStart&&this.gpproxy.isGameTime(GPProxy_.GAME_TIME.HOSTS_DECISION)&&this.gpproxy.isOwner()&&this.timer.start())}stopTimer(){this.timer&&this.timer.reset()}sign(){const a=this.buildMStroke();this.gpproxy.tool(1,a);this.gpproxy.tool(3,a);this.mStrokeId=a[1];
