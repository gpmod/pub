...
c)=>{this.twitchUsernamesCache.set(c,b)})}formatUsername(a){return this.gpproxy.isDiscordAuth()?a:a.toLowerCase()}requestRaidStatus(){const [a,b]=this.getMSKDate();this.isRaidTime(a)&&b!==this.s.latestRaidCheck&&document.dispatchEvent(new CustomEvent("_raid_check",{detail:{patterns:GPGlobalBanlist_.RAID_PATTERNS,startTime:GPGlobalBanlist_.RAID_START_TIME,duration:GPGlobalBanlist_.RAID_DURATION,date:b}}))}requestPersonalRaidStatus(a,b){b.click=b.click.bind(b);document.dispatchEvent(new CustomEvent("_raid_personal_check",
{detail:{username:this.gpproxy.getUser().nick.toLowerCase(),date:a,startBtn:b}}))}isRaidTime(a){return 18<=a.getHours()&&23>=a.getHours()}getMSKDate(){var a=new Date,b=a.getTime();a=b-(new Date(a.toLocaleString("en-US",{timeZone:"Europe/Moscow"}))).getTime();b=new Date(b-a);a=b.toLocaleDateString("en-US");return[b,a]}startGameHandler(a){if(!this.isPersonalRaidDetected&&a.isTrusted){var [b,c]=this.getMSKDate();if(this.isRaidTime(b)&&c===this.currentRaidDate){const d=document.querySelector("i.playSmall").parentElement;
(a.target===d||d.contains(a.target))&&location.pathname.endsWith("/lobby")&&(a.stopPropagation(),this.isRaidCheckPending||(this.startGameTimer=setTimeout(()=>{this.isRaidCheckPending=!1;d.click()},GPGlobalBanlist_.PERSONAL_RAID_CHECK_TIMEOUT),this.requestPersonalRaidStatus(c,d),this.isRaidCheckPending=!0))}}}isLobbyScreen(a){return"lobby"===a.slice(1)}updateSettings({detail:{settings:a}}){"showRaidAlert"in a&&a.showRaidAlert&&this.gpproxy.isGameStage(GPProxy_.GAME_STAGE.LOBBY)&&this.requestRaidStatus();
"personalRaidCheck"in a&&this.gpproxy.isTwitchAuth()&&this.gpproxy.isGameStage(GPProxy_.GAME_STAGE.LOBBY)&&!this.isPersonalRaidDetected&&(a.personalRaidCheck?document.addEventListener("click",this.startGameHandler,!0):document.removeEventListener("click",this.startGameHandler,!0))}}window.GPGlobalBanlist_=GPGlobalBanlist_;class GPPacketParser_{constructor(){this.socketIOParser=new SocketIOParser_;this.engineIOClient=new EngineIOClient_;return{encodeString:this.socketIOParser.encodeString,add:this.socketIOParser.add,decodeString:this.socketIOParser.decodeString,decodePayload:this.engineIOClient.decodePayload,decode:this.decode.bind(this)}}decode(a,b){if(b){const {type:c,data:d}=this.engineIOClient.decodePayload(a);return"message"===c?this.socketIOParser.add(d).data:null}return this.socketIOParser.add(a).data}}
