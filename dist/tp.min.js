...
b);this.data=m;this.background=n;this.auth=c;this.author=g;this.title=f;this.date=new Date(k);this.fi=k&&this.date.getTime()>GPTimelapsePlayer_.FV2_DATE?5:4;this.container=d;this.container.isPlayerRendered=!0;this.mainContainer=e;this.defaultDisplayMode=h;this.defaultDisplayModeDensity=this.getModeDensity(h);this.cache;this.canvas;this.ctx;this.isPlaying=!1;this.currentSpeed=JSON.parse(window.localStorage.getItem("gp-timelapse-player-speed"))||GPTimelapsePlayer_.SPEED;this.speed=GPTimelapsePlayer_.SPEEDS[this.currentSpeed];
...
b){const c=a.findLastIndex(g=>11===g[0]);if(~c){const g=a.slice(c+1);a=(b??[]).concat(a.slice(0,c).filter(f=>11!==f[0]));return[g,a]}return[a,b]}initCacheWorker(){let {width:a,height:b}=getComputedStyle(this.container);a=parseInt(a);b=parseInt(b);var {density:c}=this.getDensityByDimensions(a,b);let {width:g,height:f}=getComputedStyle(this.mainContainer);g=parseInt(g);f=parseInt(f);const {density:k}=this.getDensityByDimensions(g,f),{density:d}=this.getDensityByDimensions(window.screen.width,window.screen.height),
e=[...(new Set([c,k,d]))];this.worker||(c=URL.createObjectURL(new Blob([this.getFuncBody(this.cacheWorkerBody)])),this.worker=new Worker(c),URL.revokeObjectURL(c),this.worker.addEventListener("message",({data:h})=>{"worker-ready"===h.type?e.forEach(l=>{this.worker.postMessage({type:"get-cache",density:l})}):"cache-updated"===h.type&&(this.cache=h.cache,this.dispatchEvent(new CustomEvent("cache-updated",{detail:{density:h.density}})))}));this.worker.postMessage({type:"init",data:this.data,background:this.background,
draw:this.getFuncBody(this.drawFunction),fi:this.fi,CACHE_INTERVAL:GPTimelapsePlayer_.CACHE_INTERVAL,ORIGINAL_WIDTH:GPTimelapsePlayer_.ORIGINAL_WIDTH,ORIGINAL_HEIGHT:GPTimelapsePlayer_.ORIGINAL_HEIGHT,BACKGROUND_COLOR:GPTimelapsePlayer_.BACKGROUND_COLOR})}cacheWorkerBody(){function a(k){var d=b*k,e=c*k;if(!this.cache[k]){this.cache[k]=[];const l=f,m=Math.floor(self.data.length/l);d=new OffscreenCanvas(d,e);e=d.getContext("2d",{willReadFrequently:!0});let n;self.background&&(e.fillStyle=g,e.fillRect(0,
0,d.width,d.height),self.draw(e,this.background,k,!1),n=e.getImageData(0,0,d.width,d.height),e.clearRect(0,0,d.width,d.height));for(let p=0;p<m;p++){var h=p*l;h=self.data.slice(h,h+l);self.draw(e,h,k);h=e.getImageData(0,0,d.width,d.height);this.cache[k].push(h)}self.background&&this.cache[k].push(n)}}let b,c,g,f;self.data;self.draw;self.cache=[];self.addEventListener("message",({data:k})=>{"init"===k.type?(self.data=k.data,self.background=k.background,self.fi=k.fi,self.draw=new Function("a","l","f",
