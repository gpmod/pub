...
background:b,canvasTypeId:c,auth:e,meta:{author:d,title:f,date:g}},container:h,mainContainer:k},l=GPTimelapsePlayer_.DEFAULT_DISPLAY_MODE,p=!1){super();GPTimelapsePlayer_.players.add(this);this.canvasTypeId=c;this.canvasType=GPUtils_.getCanvasTypeById(c);const [n,m]=GPUtils_.getCanvasDimensionsById(c);this.density=GPTimelapsePlayer_.CANVAS_DENSITY;this.originalWidth=n;this.originalHeight=m;this.width=this.originalWidth*this.density;this.height=this.originalHeight*this.density;this.originalData=a.slice();
...
function renderPlayer({data:a,background:b,canvasTypeId:c,auth:e,meta:{author:d,title:f,date:g}}){GPTimelapsePlayer_.closeAll();mainContainer.classList.add("player");clearTitle();showLoadingSpinner();player=new GPTimelapsePlayer_({image:{data:a,background:b,canvasTypeId:c,auth:e,meta:{author:d,title:f,date:g}},container:playerContainer,mainContainer},settings.displayMode);player.addEventListener("data_loaded",({detail:{author:h,title:k,date:l}})=>{setTitle(h,k,l)});player.addEventListener("image_rendered",
...
function downloadSource(a){document.dispatchEvent(new CustomEvent("source_request",{detail:{url:a}}));showLoadingSpinner()}function parseImageData(a){const {d:b,b:c,c:e,a:d,m:{a:f,t:g,d:h}}=a;return{data:b,background:c,canvasTypeId:e,auth:d,meta:{author:f,title:g,date:h}}}function showLoadingSpinner(){document.documentElement.classList.add("loading")}function hideLoadingSpinner(){document.documentElement.classList.remove("loading")}
