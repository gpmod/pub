...
height:c}=getComputedStyle(this.container);return this.getDensityByDimensions(parseInt(b),parseInt(c)).density;case GPTimelapsePlayer_.DISPLAY_MODE.EXPANDED:const {width:g,height:f}=getComputedStyle(this.mainContainer);return this.getDensityByDimensions(parseInt(g),parseInt(f)).density;case GPTimelapsePlayer_.DISPLAY_MODE.FULLSCREEN:return this.getDensityByDimensions(window.screen.width,window.screen.height).density}}terminate(){this.pause();this.worker.terminate();this.cache=this.originalData=this.data=
...
f&&b.length>k)if(f=this.getCache(),!this.cache[this.density]&&b.at(-1).index+1<k)this.drawFunction(a,b,c,g);else{const d=Math.floor((b.at(-1).index+1)/k)-1;a.putImageData(f[d],0,0);b=b.slice(-(b.at(-1).index+1-(d+1)*k));this.drawFunction(a,b,c,g)}else this.drawFunction(a,b,c,g)}}drawFunction(a,b,c=this.density,g=this.eraserAlpha,f=!1){a.lineCap=a.lineJoin="round";a.save();if(b){let n=[];for(var k=0;k<b.length;k++){var d=b[k],e=d[0];d=(d[1],d.slice(2));if(10!=e){var h=void 0;a.save();if(8!=e){for(h=
1;h<d.length;h++)d[h]=[d[h][0]*c,d[h][1]*c];if(2<d.length)switch(e){case 1:e=d[0];a.strokeStyle=e[0];a.lineWidth=e[1];a.globalAlpha=e[2];a.lineWidth*=c;h=[d[1][0],d[1][1]];a.beginPath();a.moveTo(...h);for(var l=2;l<d.length;l++)h=[d[l][0],d[l][1]],e=[d[l-1][0],d[l-1][1]],a.quadraticCurveTo(e[0],e[1],e[0]+(h[0]-e[0])/2,e[1]+(h[1]-e[1])/2);a.lineTo(h[0],h[1]);a.stroke();break;case 2:a.strokeStyle="#FFFFFF";a.lineWidth=d[0]*c;h=[d[1][0],d[1][1]];g&&(a.globalCompositeOperation="destination-out");a.beginPath();
a.moveTo(...h);for(l=2;l<d.length;l++)h=[d[l][0],d[l][1]],e=[d[l-1][0],d[l-1][1]],a.quadraticCurveTo(e[0],e[1],e[0]+(h[0]-e[0])/2,e[1]+(h[1]-e[1])/2);a.lineTo(h[0],h[1]);a.stroke();g&&(a.globalCompositeOperation="source-over");break;case 3:e=d[0];a.strokeStyle=e[0];a.lineWidth=e[1];a.globalAlpha=e[2];a.lineWidth*=c;a.beginPath();a.moveTo(d[1][0],d[1][1]);a.lineTo(d[2][0],d[2][1]);a.stroke();break;case 4:e=d[0];a.strokeStyle=e[0];a.lineWidth=e[1];a.globalAlpha=e[2];a.lineWidth*=c;a.beginPath();a.rect(d[1][0],
d[1][1],d[2][0]-d[1][0],d[2][1]-d[1][1]);a.stroke();break;case 6:e=d[0];a.fillStyle=e[0];a.lineWidth=e[1];a.globalAlpha=e[2];a.lineWidth*=c;a.beginPath();a.rect(d[1][0],d[1][1],d[2][0]-d[1][0],d[2][1]-d[1][1]);a.fill();break;case 5:e=d[0];a.strokeStyle=e[0];a.lineWidth=e[1];a.globalAlpha=e[2];a.lineWidth*=c;e=(d[2][0]-d[1][0])/2;h=(d[2][1]-d[1][1])/2;l=Math.round(d[1][0]+e);d=Math.round(d[1][1]+h);var m=(Math.sqrt(2)-1)/3*4;a.beginPath();a.moveTo(l,d-h);a.bezierCurveTo(l+m*e,d-h,l+e,d-m*h,l+e,d);
...
document.addEventListener("paste",async a=>{a.preventDefault();if(a.clipboardData.items.length){var b=Array.from(a.clipboardData.files).find(c=>isGPFile(c));b?(a=await GPUtils_.decompressFile(b),a=parseImageData(a),renderPlayer(a)):(a=Array.from(a.clipboardData.items).find(c=>"text/plain"===c.type))&&a.getAsString(c=>{(c=c.split("?")[0])&&URL_PATTERN.test(c)&&downloadSource(c)})}});removeNoModMsg();render();handleResize();handleQuery();
function handleQuery(){if(location.search){const a=new Map;location.search.slice(1).split("&").forEach(c=>{const [g,f]=c.split("=");a.set(decodeURIComponent(g),decodeURIComponent(f))});let b=a.get("url")?.split("?")[0];b.startsWith(`${GP_IMAGE_PROTOCOL}://`)&&(b=b.slice(GP_IMAGE_PROTOCOL.length+3));b&&URL_PATTERN.test(b)&&downloadSource(b)}}
