...
f=k.CACHE_INTERVAL,b=k.ORIGINAL_WIDTH,c=k.ORIGINAL_HEIGHT,g=k.BACKGROUND_COLOR,self.postMessage({type:"worker-ready"})):"get-cache"===k.type&&(self.cache[k.density]||a(k.density),self.postMessage({type:"cache-updated",cache:self.cache,density:k.density}))})}getFuncBody(a){a=a.toString();return a.substring(a.indexOf("{")+1,a.lastIndexOf("}"))}}window.GPTimelapsePlayer_=GPTimelapsePlayer_;const GP_IMAGE_PROTOCOL="web+gpimg",GP_FILE_FORMAT="gpimg",URL_PATTERN=new RegExp(`^https?:\\/\\/.+\\.${GP_FILE_FORMAT}$`,"i"),L10N={ru:{DROP_ZONE_MSG:["\u041f\u0435\u0440\u0435\u043d\u0435\u0441\u0438\u0442\u0435 \u0438\u0441\u0445\u043e\u0434\u043d\u0438\u043a \u0438\u0437\u043e\u0431\u0440\u0430\u0436\u0435\u043d\u0438\u044f \u043d\u0430 \u0441\u0442\u0440\u0430\u043d\u0438\u0446\u0443 \u0438\u043b\u0438","\u0432\u044b\u0431\u0435\u0440\u0438\u0442\u0435 \u0444\u0430\u0439\u043b","\u0441 \u043a\u043e\u043c\u043f\u044c\u044e\u0442\u0435\u0440\u0430"]},
...
document.addEventListener("source_request_complete",async({detail:{success:a,blob:b}})=>{a?(a=await GPUtils_.decompressFile(b),a=parseImageData(a),renderPlayer(a)):hideLoadingSpinner()});"launchQueue"in window&&"files"in LaunchParams.prototype&&launchQueue.setConsumer(async a=>{a.files.length&&(a=await a.files[0].getFile(),a=new File([a],a.name||"",{type:a.type}),isGPFile(a))&&(a=await GPUtils_.decompressFile(a),a=parseImageData(a),renderPlayer(a),history.replaceState({},document.title,location.pathname))});
